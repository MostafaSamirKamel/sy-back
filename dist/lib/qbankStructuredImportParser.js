...
function parseFields(block) {
    ...
    const flush = () => {
        if (currentKey)
            fields.set(currentKey, currentLines.join('\n').trim());
    };
...
function sanitizeReferenceName(raw) {
    const cleaned = (raw || '')
        .replace(/\n+/g, ' ')
        .replace(/-{3,}/g, ' ')
        .replace(/\s+/g, ' ')
...
function parseCorrectIndex(raw, options) {
    const trimmed = raw.trim();
    const letterMatch = trimmed.match(/^([A-D])\b/i);
    if (letterMatch)
        return letterMatch[1].toUpperCase().charCodeAt(0) - 65;
    const textMatch = options.findIndex((opt) => opt.toLowerCase() === trimmed.toLowerCase() || trimmed.toLowerCase().includes(opt.toLowerCase()));
    return textMatch >= 0 ? textMatch : -1;
}
function parseTags(raw, fields) {
    const tags = new Set();
    const add = (value) => {
        if (!value?.trim())
            return;
...
function buildQuestionText(fields) {
...
function buildExplanation(fields) {
    ...
    if (learningPoint)
        parts.push(`Learning Point: ${learningPoint}`);
    if (pearl)
        parts.push(`High-Yield Pearl: ${pearl}`);
    if (whyWrong)
        parts.push(`Why Others Are Wrong:\n${whyWrong}`);
    if (tags.length)
        parts.push(`Tags: ${tags.join(', ')}`);
    if (system)
        parts.push(`System: ${system}`);
    if (estimatedTime)
        parts.push(`Estimated Time: ${estimatedTime}`);
    if (bloomLevel)
        parts.push(`Bloom Level: ${bloomLevel}`);
    if (topic)
...
export function parseStructuredQbankImport(source) {
