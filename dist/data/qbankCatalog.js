...
const MODULES_BY_TERM = {
    '401': QBANK_MODULES_401,
};
export function getQbankTerm(termId) {
    return QBANK_TERMS.find((t) => t.id === termId);
}
export function getQbankModules(termId) {
    return MODULES_BY_TERM[termId] ?? [];
}
export function getQbankModule(termId, moduleId) {
    return getQbankModules(termId).find((m) => m.id === moduleId);
}
export function isPurchasableModule(termId, moduleId) {
