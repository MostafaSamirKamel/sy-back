...
async function main() {
    ...
    for (const s of specialties) {
        ...
        if (!existing)
            await prisma.specialty.create({ data: s });
    }
    const difficulties = [
            ...
            data: [
                {
                    ...
                    content: 'Ask about onset, character, radiation, associated dyspnea, palpitations, syncope, risk factors (smoking, family history), and exertional symptoms.',
                    type: 'QUESTION',
                },
                {
                    categoryId: chest.id,
                    ...
                    content: 'Students should explore history of rheumatic fever, penicillin prophylaxis, murmur characteristics, and functional limitation.',
                    type: 'TEACHING',
                },
            ],
        });
    ...
    if (cardiology && advanced) {
        ...
        const caseData = {
            ...
            medicationHistory: 'Depot penicillin IM every 15 days (prophylaxis). Inhaled Ventolin (salbutamol) for asthma. Occasional home oxygen for asthma attacks.',
            surgicalHistory: 'Denies prior surgery — but has a chest tube scar on inspection (critical discrepancy for students to identify).',
            familyHistory: 'No consanguinity. No first-degree relatives with congenital or valvular heart disease.',
            socialHistory: 'Single 17-year-old male from Shobra Al-Kheima, Cairo. Apprentice house painter (chemical fume exposure). Non-smoker.',
            physicalExam: JSON.stringify({
                inspection: 'Mildly tachypneic cooperative adolescent sitting upright. Evident linear hyperpigmented scar from previous chest tube insertion (past pleural effusion) in left mid-axillary line. No precordial bulge, no dilated superficial veins, no chest skin lesions.',
                palpation: 'Apex in left 5th ICS, displaced laterally from midclavicular line — sustained heaving apex (LV concentric hypertrophy). No epigastric, suprasternal, or parasternal pulsations. Distinct rough systolic thrill at 1st aortic area (R 2nd ICS) radiating to carotids.',
                percussion: 'Normal cardiac dullness. Lungs resonant bilaterally; possible mild dullness at left lung base near old chest tube scar.',
                auscultation: 'Aortic area: muffled/soft S2; loud ejection systolic murmur at 1st aortic area radiating to carotids and apex (louder sitting forward, breath held in expiration). Mitral area: soft muffled S1; loud blowing pansystolic murmur at apex radiating to left axilla (louder in left lateral decubitus). Mild expiratory wheeze bilaterally (asthma).',
            }),
                ...
                sections: [
                    {
                        ...
                        content: 'Hb 12.8 g/dL. WBC 9.6×10⁹/L (mildly elevated — recent tonsillitis). Platelets 280,000. ESR 45 mm/hr (elevated). CRP positive.',
                        contentAr: 'Hb 12.8. WBC 9.6 (مرتفع قليلاً). صفائح 280,000. ESR 45 (مرتفع). CRP إيجابي.',
                    },
                    {
                        title: 'ECG',
                        titleAr: 'رسم القلب',
                        content: 'Sinus rhythm 90 bpm. LVH with strain (deep S in V1–V2, tall R in V5–V6, ST depression/T inversion lateral). P-mitrale in lead II (LAE).',
                        contentAr: 'إيقاع sinus 90. LVH مع strain. P-mitrale (تضخم أذين أيسر).',
                    },
                    {
                        title: 'Echocardiography',
                        titleAr: 'إيكو القلب',
                        content: 'Severe AS: calcified rheumatic aortic valve, PG 64 mmHg, AVA 0.8 cm². Double mitral disease: moderate MR + moderate MS (MVA 1.6 cm²). Concentric LVH, LVEF 60%. No pleural effusion.',
                        contentAr: 'تضيق aortic شديد PG 64، AVA 0.8. مرض mitral مزدوج: MR + MS متوسط. LVH متحد المركز EF 60%.',
                    },
                    {
                        title: 'Chest X-Ray',
                        titleAr: 'أشعة الصدر',
                        content: 'Boot-shaped heart, enlarged LV, prominent LAA. Mild hyperinflation (asthma). Pleural thickening at right mid-axillary margin (prior chest tube).',
                        contentAr: 'قلب boot-shaped. تضخم LV. hyperinflation خفيف. تثخن pleural عند موقع أنبوب الصدر.',
                    },
                ],
            }),
            finalDiagnosis: 'Rheumatic combined valvular heart disease — severe aortic stenosis and mitral regurgitation, complicated by bronchial asthma',
            ...
            patientPersonality: 'Anxious 17-year-old painter, cooperative but minimizes surgical history. Speaks Egyptian Arabic.',
...
Do NOT volunteer the diagnosis. Answer in Egyptian Arabic when student uses Arabic.`,
    ...
    if (cardiology && beginner) {
        ...
        const hfCaseData = {
            ...
            patientSmokingStatus: 'Former smoker (quit 5 years ago)',
            ...
            physicalExam: 'Bilateral basal crepitations. Raised JVP. Bilateral pitting ankle edema. Displaced apex beat. S3 gallop. No murmur.',
            examImages: JSON.stringify([
                {
            ...
            labResults: JSON.stringify({
                sections: [
                    ...
                    },
                    {
                        title: 'Echocardiography',
                        titleAr: 'إيكو القلب',
                        content: 'Severely reduced LV ejection fraction (~30%). Dilated left ventricle. Moderate functional mitral regurgitation.',
            ...
            teachingPoints: 'Recognize heart failure (dyspnea, orthopnea, PND, edema). Interpret BNP and echo. Know acute management with diuretics and afterload reduction.',
            evaluationRubric: 'Introduction (10%), Presenting complaint (20%), Cardiac history (25%), Examination (20%), Diagnosis & management (15%), Professionalism (10%)',
            vitalSigns: JSON.stringify({
            ...
            patientPersonality: 'Anxious older woman, breathless when speaking, cooperative but fatigued.',
            scenarioPrompt: 'Background: 58-year-old Egyptian woman with breathlessness and ankle swelling for 3 weeks. Do not mention any details unless the doctor asks.',
            isPublished: true,
        };
        if (!existingHfCase) {
