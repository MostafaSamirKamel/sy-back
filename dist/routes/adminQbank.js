...
import { parseOptions, parseSubjects, validateQuestionInput, grantModuleAccess, } from '../services/qbankService.js';
import { syncModuleUniversities } from '../lib/universityScope.js';
import { parseStructuredQbankImport, sanitizeImportedReferenceName, } from '../lib/qbankStructuredImportParser.js';
const router = Router();
...
async function buildImportTemplateCsv() {
    const defaults = {
        termId: '401',
        moduleId: 'med-1',
    ...
    try {
        ...
        if (mod?.id)
            moduleId = mod.id;
        if (chapters.length)
            chapterNames = chapters.map((c) => c.nameEn);
        if (references.length)
            referenceNames = references.map((r) => r.nameEn);
    }
    catch (error) {
        console.warn('[qbank template] Using static defaults:', error);
    }
    const rows = [Array.from(CSV_COLUMNS)];
...
async function ensureReferenceId(name, cache) {
...
async function validateStructuredQuestion(question, rowNum, termId, moduleId, autoCreateLookups, lookupCache, subjectOverride) {
    const errors = [];
    const mod = await prisma.qbankModule.findFirst({
        where: { id: moduleId, termId, isActive: true },
    });
    if (!mod)
        errors.push(`Module ${moduleId} not found in term ${termId}`);
    errors.push(...validateQuestionInput({
        text: question.text,
    ...
    if (question.chapter.trim()) {
        if (autoCreateLookups) {
            ...
            if (!chapterId)
                errors.push(`Chapter "${question.chapter}" not found`);
        }
    }
    ...
    if (autoCreateLookups) {
        ...
        if (!referenceId)
            errors.push(`Reference "${referenceName}" not found`);
    }
    ...
    return {
        ...
        errors,
        data: errors.length === 0 && mod && chapterId && referenceId
            ? {
                moduleId: mod.id,
                chapterId,
                referenceId,
...
async function validateImportRow(row, rowNum) {
    const errors = [];
    ...
    if (!termId)
        errors.push('termId is required');
    if (!moduleId)
...
router.post('/terms/:termId/modules', async (req, res) => {
    const { id, nameEn, nameAr, specialtyEn, specialtyAr, subjects = [], free = false, bundled = false, priceEgp = 50, sortOrder = 0, isActive = true, universityIds = [], } = req.body;
    ...
    try {
        await syncModuleUniversities(mod.id, Array.isArray(universityIds) ? universityIds.map(String) : []);
    }
    catch (error) {
...
router.put('/modules/:id', async (req, res) => {
    ...
    const { nameEn, nameAr, specialtyEn, specialtyAr, subjects, free, bundled, priceEgp, sortOrder, isActive, universityIds, } = req.body;
...
router.post('/chapters', async (req, res) => {
    ...
    if (!nameEn?.trim())
        return res.status(400).json({ error: 'nameEn is required' });
...
router.get('/questions', async (req, res) => {
    ...
    if (req.query.moduleId)
        where.moduleId = String(req.query.moduleId);
    if (req.query.chapterId)
        where.chapterId = String(req.query.chapterId);
    if (req.query.referenceId)
        where.referenceId = String(req.query.referenceId);
    if (req.query.termId)
        where.module = { termId: String(req.query.termId) };
    if (req.query.search) {
...
router.post('/questions', async (req, res) => {
    const { moduleId, chapterId, referenceId, text, options, correctIndex, explanation, subjectTags, isPublished = true, sortOrder = 0, } = req.body;
...
router.put('/questions/:id', async (req, res) => {
    ...
    const { moduleId, chapterId, referenceId, text, options, correctIndex, explanation, subjectTags, isPublished, sortOrder } = req.body;
...
router.post('/questions/import/structured/preview', async (req, res) => {
    ...
    if (!termId || !moduleId)
        return res.status(400).json({ error: 'termId and moduleId are required' });
    ...
    for (let i = 0; i < questions.length; i += 1) {
        results.push(await validateStructuredQuestion(questions[i], i + 1, termId, moduleId, autoCreateLookups, lookupCache, subject || undefined));
    }
    const valid = results.filter((r) => r.valid);
    const invalid = results.filter((r) => !r.valid);
    res.json({
