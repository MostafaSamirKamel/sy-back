...
import { caseToForm, formToCaseData, parseStationConfigForm, } from "../services/caseFormService.js";
import { importedCaseToForm, parseImportedCaseSource, } from "../lib/caseImportParser.js";
import { examCasesUploadRoot, ensureExamMediaDirs, } from "../lib/examMediaPaths.js";
import { mergeStationConfig, parsePartialStationConfig, parseStationConfig, serializePartialStationConfig, } from "../lib/stationConfig.js";
const router = Router();
ensureExamMediaDirs();
...
router.post("/media/upload", async (req, res) => {
    const { fileName, mimeType, dataBase64, caseSlug } = req.body;
...
router.put("/:id/university-overrides/:universityId", async (req, res) => {
    ...
    const patch = req.body?.stationConfig;
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        // Explicitly clear dependents (also covered by Session→Case cascade after migration).
        // Keeps deletes reliable even when FK cascade is not yet applied on production MySQL.
        await prisma.$transaction(async (tx) => {
            ...
            if (sessionIds.length > 0) {
                ...
                await tx.aiUsageLog.deleteMany({ where: { sessionId: { in: sessionIds } } });
