...
router.post('/verify-otp', [body('email').isEmail().normalizeEmail(), body('code').trim().isLength({ min: 6, max: 6 })], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    ...
    if (!result.ok) {
        const error = result.reason === 'EXPIRED' ? 'Verification code expired' : 'Invalid verification code';
...
router.put('/profile', authenticate, async (req, res) => {
    ...
    const resolvedUniversity = universityId !== undefined || university !== undefined
