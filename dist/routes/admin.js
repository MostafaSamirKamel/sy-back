...
import { getPlanDefinition, getUserEntitlements, getActiveSubscription, setUserSubscriptionPlan, listPlanConfigs, clearPlanCache, ensurePlanConfigsSeeded, } from '../services/subscriptionService.js';
import { clearAISettingsCache } from '../services/aiService.js';
import { getAiUsageSummary, getCostRatesMap, clearCostRatesCache, ensureDefaultCostRates, } from '../services/aiUsageService.js';
...
router.get('/stats', async (req, res) => {
    ...
    const [usersTotal, cases, sessionsInRange, completedInRange, newUsersInRange, avgScoreInRange, revenueAgg, allTimeSessions, allTimeCompleted, allTimeAvg, recentSessions, sessionsForSeries, usersForSeries, paymentsForSeries,] = await Promise.all([
        prisma.user.count(),
        prisma.case.count(),
...
router.get('/users', async (req, res) => {
    ...
    if (plan && SUBSCRIPTION_PLANS.includes(plan)) {
        if (plan === 'FREE') {
            ...
            and.push({ subscriptions: { some: { status: 'ACTIVE', plan: plan } } });
        }
    }
    if (and.length)
        where.AND = and;
...
router.get('/users/:userId/profile', async (req, res) => {
    ...
    const [entitlements, subscription, qbankModules, recentSessions, completedCount, results] = await Promise.all([
...
router.patch('/users/:id', async (req, res) => {
    ...
    if (academicYear !== undefined)
        data.academicYear = academicYear || null;
...
router.put('/ai-settings', async (req, res) => {
    ...
    if (!settings)
        settings = await prisma.aISettings.create({ data: {} });
    const { provider, patientModel, examinerModel, temperature, maxTokens, systemPromptAr, systemPromptEn, patientSystemPromptAr, patientSystemPromptEn, examinerSystemPromptAr, examinerSystemPromptEn, maxContextMessages, openRouterApiKey, } = req.body;
...
router.post('/users/:userId/subscription', async (req, res) => {
    ...
    try {
        ...
        const subscription = await setUserSubscriptionPlan(user.id, plan, {
