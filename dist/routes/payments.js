...
import { completeMockCheckout, createCheckout, createModuleCheckout, getOrderForUser, getPaymentPublicConfig, getPaymobReturnUrl, getKashierReturnUrl, handlePaymobReturn, handlePaymobWebhook, handleKashierReturn, handleKashierWebhook, isCheckoutPlanId, isPaymentEnabled, } from '../services/payment/paymentService.js';
const router = Router();
...
router.post('/checkout', authenticate, [body('planId').isString().notEmpty()], async (req, res) => {
...
router.post('/checkout-module', authenticate, [body('termId').isString().notEmpty(), body('moduleId').isString().notEmpty()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    if (!isPaymentEnabled()) {
...
router.get('/orders/:merchantOrderId', authenticate, async (req, res) => {
...
router.get('/return/paymob', async (req, res) => {
    const query = Object.fromEntries(Object.entries(req.query).map(([key, value]) => [key, Array.isArray(value) ? value[0] : value]));
    try {
...
router.get('/return/kashier', async (req, res) => {
    const query = Object.fromEntries(Object.entries(req.query).map(([key, value]) => [key, Array.isArray(value) ? value[0] : value]));
    try {
...
router.post('/webhook/kashier', async (req, res) => {
    try {
