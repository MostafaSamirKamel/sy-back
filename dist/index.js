...
import { ensureExamMediaDirs, persistentExamRoot, packagedExamRoot, } from "./lib/examMediaPaths.js";
import { ensureAiKnowledgeDirs, persistentAiKnowledgeRoot, } from "./lib/aiKnowledgePaths.js";
...
app
    ...
    if (isPaymentEnabled()) {
        ...
        if ((requested === "paymob" && provider === "mock") ||
            ...
            console.log(`[payments] ${requested} not configured — using instant mock activation until gateway is connected`);
        }
        else {
        ...
        console.warn("[payments] Not configured — set PAYMENT_PROVIDER=kashier (or paymob) and the matching gateway keys");
    }
})
    .on("error", (err) => {
