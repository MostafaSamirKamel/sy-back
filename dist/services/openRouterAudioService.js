...
export class OpenRouterAudioError extends Error {
    ...
    code;
    upstreamStatus;
    constructor(stage, code, upstreamStatus) {
        super(code);
        this.stage = stage;
        this.code = code;
...
function sanitizeTranscriptForLog(text) {
    return text
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 240);
...
function fetchErrorDetails(error, timedOut) {
...
export function openRouterAudioInputFormat(mimeType) {
...
export function resolveOpenRouterAudioVoice(patientGender) {
...
function isStandaloneThanks(text) {
    return /^(شكراً|شكرا|thanks?|shukran)[.!؟?]*$/i.test(text.trim());
}
function stripLikelyTrailingSttOutro(text) {
...
export function salvageOpenRouterTranscript(text) {
...
function normalizeEgyptianVoiceSpacing(text) {
...
export function normalizeOpenRouterTranscript(text, language, forceArabic = false, speechDurationMs, byteSize) {
 ...
 */
export async function transcribeOpenRouterAudioWithDiagnostics(input) {
    const key = process.env.OPENROUTER_API_KEY?.trim();
    if (!key)
        throw new Error('openrouter-audio-unavailable');
