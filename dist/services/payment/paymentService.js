...
import { createPaymobCheckout, isPaymobConfigured, isPaymobSuccess, verifyPaymobTransactionHmac, } from './paymobProvider.js';
import { createKashierCheckoutUrl, isKashierConfigured, isKashierOrderCaptured, isKashierSuccess, normalizeKashierPayload, verifyKashierSignature, } from './kashierProvider.js';
const PAID_PLAN_IDS = ['PACKAGE_50', 'PACKAGE_150', 'PACKAGE_300'];
...
export function isCheckoutPlanId(planId) {
    return PAID_PLAN_IDS.includes(planId);
}
export async function createCheckout(userId, planId) {
...
export async function createModuleCheckout(userId, termId, moduleId) {
...
export async function completeMockCheckout(merchantOrderId, userId) {
...
export async function getOrderForUser(merchantOrderId, userId) {
    const order = await prisma.paymentOrder.findUnique({ where: { merchantOrderId } });
    if (!order || order.userId !== userId)
        return null;
    ...
    const planConfig = order.plan ? PLAN_CATALOG[order.plan] : undefined;
    return {
        merchantOrderId: order.merchantOrderId,
        status: order.status,
...
async function finalizePaidOrder(orderId, transactionId) {
...
async function finalizeFailedOrder(orderId, reason) {
...
export async function handlePaymobReturn(query) {
...
export function getPaymobReturnUrl() {
    return `${serverBaseUrl()}/api/payments/return/paymob`;
}
export function getKashierReturnUrl() {
    return `${serverBaseUrl()}/api/payments/return/kashier`;
}
export function getKashierWebhookUrl() {
    return `${serverBaseUrl()}/api/payments/webhook/kashier`;
}
export async function handleKashierReturn(query) {
...
export async function handleKashierWebhook(body) {
    const { params, signature } = normalizeKashierPayload(body);
    const merchantOrderId = String(params.merchantOrderId || params.orderId || params.paymentRequestId || '');
    ...
    return { ok: true, status: order.status.toLowerCase() };
}
