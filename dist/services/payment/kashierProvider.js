...
export function isKashierConfigured() {
    return !!(process.env.KASHIER_MID &&
        process.env.KASHIER_API_KEY &&
        process.env.KASHIER_SECRET_KEY);
}
export function isKashierTestMode() {
...
export function createKashierCheckoutUrl(params) {
    ...
    const allowedMethods = process.env.KASHIER_ALLOWED_METHODS || 'card,wallet,bank_installments';
 ...
 */
export function verifyKashierSignature(params, receivedSignature) {
...
function buildSignatureCandidates(params) {
    const entries = Object.entries(params).filter(([key]) => key !== 'signature' && key !== 'mode');
...
export function isKashierSuccess(status) {
 ...
 */
export async function isKashierOrderCaptured(merchantOrderId) {
    if (!isKashierConfigured())
        return false;
    const secretKey = process.env.KASHIER_SECRET_KEY || '';
    try {
        const res = await fetch(`${kashierApiBase()}/payments/orders/${encodeURIComponent(merchantOrderId)}`, {
            method: 'GET',
            headers: {
        ...
        const status = data?.response?.status || data?.data?.status || data?.status || '';
 ...
 */
export function normalizeKashierPayload(body) {
    let params = { ...body };
