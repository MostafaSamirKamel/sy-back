...
const ENGLISH_WHISPER_PROMPT = 'OSCE medical interview. Non-native English speaker. Hello doctor. What is your name? How old are you? What brings you here today? Where is the pain?';
...
export function looksLikePromptHallucination(text) {
...
export function extractPrimaryUtterance(text, allowLatinOnly = false) {
...
export function resolveWhisperLanguage(language, forceArabic) {
    if (forceArabic)
        return 'ar';
    const code = language.toLowerCase().trim();
    if (code === 'auto' || code === 'auto-detect')
        return 'auto';
...
function transcriptionLooksWrong(text, expected) {
    if (expected === 'auto')
        return false;
...
function needsWhisperArabicFallback(text, lang) {
    if (lang === 'auto')
        return false;
    if (lang !== 'ar')
        return transcriptionLooksWrong(text, lang);
    if (containsWrongScriptForArabic(text))
        return true;
...
async function runWhisper(client, buffer, mimeType, whisperLang, model, usePrompt) {
...
function finalizeTranscript(text, expectArabic, options) {
...
async function transcribeWithOpenAI(buffer, mimeType, language, forceArabic, options) {
    ...
    async function attempt(lang, model, usePrompt) {
        try {
...
async function transcribeWithOpenAISafe(buffer, mimeType, language, forceArabic, options) {
    try {
        ...
        try {
            console.warn('[stt] primary OpenAI path failed (%s) — last-chance whisper-1/auto', message || openAiError.code || openAiError.status || 'unknown');
            const client = getOpenAIClient();
            ...
            if (fb === 'transcription-prompt-leak' ||
 ...
 */
async function assertAudioHasSpeech(buffer, mimeType) {
    try {
...
export async function transcribeAudioBuffer(buffer, mimeType, language, forceArabic, options) {
    ...
    if (sttProvider === 'local') {
        try {
            ...
            const canFallbackToOpenAI = !!process.env.OPENAI_API_KEY?.trim() &&
                (process.env.AI_PROVIDER || 'openai').toLowerCase() !== 'mock';
            if (canFallbackToOpenAI) {
                console.warn('[stt] local Whisper failed (%s) — falling back to OpenAI transcription', message || (err instanceof Error ? err.name : 'unknown'));
