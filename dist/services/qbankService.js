...
function toModuleView(mod, purchased) {
...
export async function grantModuleAccess(userId, termId, moduleId) {
...
export async function getUserEntitlements(userId, termId) {
...
export async function getModuleFromDb(termId, moduleId, universityId) {
    const visibility = universityId !== undefined ? qbankModuleUniversityFilter(universityId) : {};
    return prisma.qbankModule.findFirst({
        where: { id: moduleId, termId, isActive: true, ...visibility },
    });
}
export async function userHasModuleAccess(userId, termId, moduleId) {
...
export async function isPurchasableModule(userId, termId, moduleId) {
    const universityId = await resolveUserUniversityId(userId);
    const mod = await getModuleFromDb(termId, moduleId, universityId);
    return !!mod && !mod.free;
}
export async function getActiveTerms(userId) {
    const universityId = await resolveUserUniversityId(userId);
...
export async function getModulesForUser(userId, termId) {
    const universityId = await resolveUserUniversityId(userId);
    const moduleWhere = { isActive: true, ...qbankModuleUniversityFilter(universityId) };
...
export async function getModuleSetupMeta(termId, moduleId) {
    ...
    return {
        ...
        pairCounts: chapters.flatMap((chapter) => references.map((reference) => ({
...
export async function countAvailableQuestions(moduleId, chapterIds, referenceIds, subjectTags) {
    if (chapterIds.length === 0 || referenceIds.length === 0)
        return 0;
    ...
    return questions.filter((q) => {
        if (!q.subjectTags)
            return true;
        try {
            const tags = JSON.parse(q.subjectTags);
...
export async function fetchExamQuestions(moduleId, filters) {
    const limit = Math.min(Math.max(1, filters.count), MAX_EXAM_QUESTIONS);
    if (filters.chapterIds.length === 0 || filters.referenceIds.length === 0)
        return [];
    ...
    return selected.map((q) => {
        ...
        if (q.subjectTags) {
            try {
                const parsed = JSON.parse(q.subjectTags);
                if (Array.isArray(parsed))
                    subjectTags = parsed.map(String);
            }
            catch {
