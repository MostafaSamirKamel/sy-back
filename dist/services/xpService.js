...
function overallPerformanceXp(totalScore) {
    return roundXp(clamp(totalScore, 0, 100) * 0.2);
}
/** Examiner viva accuracy (exam + diagnosis answers) → 0–10 XP */
function examinerQuestionsXp(clinicalReasonScore, closingScore) {
...
function excellentHistoryXp(historyTakingScore, communicationScore) {
    const avg = (historyTakingScore + communicationScore) / 2;
    return avg >= EXCELLENT_HISTORY_THRESHOLD ? 10 : 0;
}
export function calculateXpBreakdown(scores) {
    const lines = [
...
export async function isRepeatCaseCompletion(userId, caseId, sessionId) {
...
export async function applySessionXp(userId, sessionId, caseId, scores) {
