...
export async function ensurePlanConfigsSeeded() {
    ...
    for (const [plan, legacyQuota] of Object.entries(LEGACY_PLAN_QUOTAS)) {
        const nextQuota = PLAN_CATALOG[plan].casesQuota;
        if (nextQuota === legacyQuota)
            continue;
...
export function getPlanConfig(plan) {
    const key = plan in PLAN_CATALOG ? plan : 'FREE';
    if (planCache?.[key])
        return planCache[key];
    return PLAN_CATALOG[key];
}
export async function getPlanDefinition(plan) {
...
async function resolvePlanEndDate(subscription) {
...
async function getEffectiveAttempts(userId, caseId) {
...
async function getUsedCaseCredits(userId, since) {
...
export async function checkCanStartCase(userId, caseId) {
    const subscription = await getActiveSubscription(userId);
    const plan = subscription?.plan ?? 'FREE';
    const config = await getPlanDefinition(plan);
    if (!isPaidPlan(plan)) {
...
export async function recordCaseAttempt(userId, caseId) {
...
export async function pickRandomEligibleCase(userId, categoryIds, categoryRootId) {
    const subscription = await getActiveSubscription(userId);
    const plan = subscription?.plan ?? 'FREE';
    ...
    const ids = (Array.isArray(categoryIds) ? categoryIds : categoryIds ? [categoryIds] : [])
        .map((id) => id.trim())
        .filter(Boolean);
    // Dashboard "major specialties" are stored as KnowledgeCategory roots.
    // Resolve the selected root and its descendants from structured data rather
    // than guessing from case titles or confusing it with finer Case.specialty.
    let scopedCategoryIds = null;
    if (categoryRootId?.trim()) {
        ...
        for (const category of categories) {
            if (!category.parentId)
                continue;
            const idsForParent = children.get(category.parentId) ?? [];
            idsForParent.push(category.id);
            children.set(category.parentId, idsForParent);
        }
        const root = categoryRootId.trim();
        if (!categories.some((category) => category.id === root)) {
            scopedCategoryIds = [];
        }
        else {
            const pending = [root];
    ...
    const cases = await prisma.case.findMany({
        where: {
            ...
            ...(scopedCategoryIds ? { categoryId: { in: scopedCategoryIds } } : {}),
            ...(ids.length === 1 ? { categoryId: ids[0] } : ids.length > 1 ? { categoryId: { in: ids } } : {}),
        },
        include: { specialty: true, difficulty: true, category: true },
    });
    if (cases.length === 0) {
        return { ok: false, code: 'NO_CASES' };
    }
    const eligible = [];
    for (const caseData of cases) {
        const access = await checkCanStartCase(userId, caseData.id);
        if (access.allowed)
            eligible.push(caseData);
    }
    if (eligible.length === 0) {
        return { ok: false, code: 'NO_ELIGIBLE_CASES' };
    }
    // Avoid immediately repeating the student's most recent case when
    // another eligible case is available.
    const latestSession = await prisma.session.findFirst({
        where: { userId },
        orderBy: { startedAt: 'desc' },
        select: { caseId: true },
    });
    const nonRepeated = eligible.length > 1 && latestSession?.caseId
        ? eligible.filter((caseData) => caseData.id !== latestSession.caseId)
        : eligible;
    const pool = nonRepeated.length > 0 ? nonRepeated : eligible;
    const picked = pool[Math.floor(Math.random() * pool.length)];
    return { ok: true, case: picked, eligibleCount: eligible.length };
}
export async function activatePlan(userId, plan, options) {
    if (plan === 'FREE') {
...
export async function setUserSubscriptionPlan(userId, plan, options) {
    if (plan === 'FREE') {
        await prisma.subscription.updateMany({
            where: { userId, status: 'ACTIVE' },
            data: { status: 'CANCELLED', endDate: new Date() },
