...
export function fixWellbeingMishearing(text) {
...
export function prioritizeWellbeingTranscript(text) {
    ...
    if (/أخبار|اخبار|إزيك|ازيك|عامل إيه|عاملة إيه/i.test(trimmed)) {
        ...
        if (wellbeingPart)
            return `${wellbeingPart}؟`;
    }
    return trimmed;
}
/** Whisper / gpt-transcribe filler on silence — not real student speech. */
export function looksLikeSttHallucination(text, allowLatinOnly = false) {
    const normalized = text.trim().replace(/\s+/g, ' ');
    if (!normalized || normalized.length < 2)
        return true;
    if (containsWrongScriptForArabic(normalized))
        return true;
    if (/شكرا?\s*(للمشاركة|على المشاهدة|لمشاهدتك|للاستماع|جزيلا)|thank\s*you\s*for\s*(watching|listening|your)|subscribe|subtitles\s*by|مترجم|amara\.org/i.test(normalized)) {
        return true;
    }
    if (/اشترك(وا|و|ي)?\s*(في|فى)\s*(ال)?قناة|لا\s*تنس(وا|و|ي)?\s*(ال)?اشتراك|فعل(وا|و|ي)?\s*زر\s*(ال)?جرس|(اضغط|اضغطوا|دوس)(وا|و|ي)?\s*(على\s*)?زر\s*(ال)?(اشتراك|جرس)|subscribe\s*(to\s*)?(the\s*)?channel|like\s*(and\s*)?subscribe/i.test(normalized)) {
        return true;
    }
    ...
    if (/^(buch\.?|hello[,.]?\s*world!?|sorry[,.]?\s*could you clarify\??|thank you for (watching|listening)|thanks for watching|subtitle(s)? by|www\.|http)/i.test(normalized)) {
        return true;
    }
    if (/^(um+|uh+|hmm+|ah+|oh+|mm+|mhm)[.!?]*$/i.test(normalized) ||
...
export function containsWrongScriptForArabic(text) {
    return /[\u4e00-\u9fff\u3400-\u4dbf\u3040-\u30ff\uac00-\ud7af]/.test(text);
}
export function isValidArabicSessionTranscript(text, expectArabic) {
...
export function fixArabicSpeechTranscript(text, expectArabic, 
/** AUTO mode: map known English Whisper mishearings of Egyptian OSCE phrases. */
codeSwitch = false) {
    const normalized = text.trim().replace(/\s+/g, ' ');
    if (!normalized)
        return normalized;
    if (!expectArabic && !codeSwitch)
        return normalized;
    const wellbeingFixed = fixWellbeingMishearing(normalized);
    if (wellbeingFixed !== normalized)
        return wellbeingFixed;
...
export function transcriptionNeedsArabicFix(text, expectArabic) {
    if (!expectArabic)
        return false;
    const normalized = text.trim();
    if (!normalized)
...
export function shouldForceArabicTranscription(sessionLang) {
