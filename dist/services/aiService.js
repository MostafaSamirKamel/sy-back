...
import { formatPatientBehaviorPrompt, parseStationConfig, resolveManeuverOpeningMessage, resolveManeuverLabel, } from '../lib/stationConfig.js';
function formatStageLabel(stage) {
    if (stage === 'history')
        return 'History — Patient Interview';
    if (stage === 'history:examiner')
        return 'History — Examiner Viva';
...
export function buildSessionTranscript(caseData, messages) {
 ...
 */
export function guardExaminerEvaluationScores(ai, baseline) {
    const guard = (aiScore, baselineScore) => {
    ...
    const communicationScore = guard(ai.communicationScore, baseline.communicationScore);
    const historyTakingScore = guard(ai.historyTakingScore, baseline.historyTakingScore);
    const clinicalReasonScore = guard(ai.clinicalReasonScore, baseline.clinicalReasonScore);
    const organizationScore = guard(ai.organizationScore, baseline.organizationScore);
    const closingScore = guard(ai.closingScore, baseline.closingScore);
    const totalScore = Math.round(communicationScore * 0.2 +
...
function buildPatientSystemPrompt(caseData, language, knowledgeContext, voiceTurn = false, studentTurn = 0, patientBehavior) {
...
function mergeEvaluationWithLanguage(ai, localized, lang) {
    if (lang === 'EN')
        return ai;
    const pick = (aiText, localizedText) => textHasArabic(aiText) ? aiText : localizedText;
    return {
        ...ai,
...
export function resolveEvaluationLanguage(sessionLang, messages, uiLang) {
...
function buildExaminerEvaluationPrompt(caseData, knowledgeContext, lang) {
...
async function getAISettings() {
    ...
    if ((settings.systemPromptAr || settings.systemPromptEn) &&
    ...
    return {
        ...
        patientModel: process.env.OPENAI_PATIENT_MODEL ||
...
function adminSystemPromptSuffix(settings, lang, role = 'patient') {
    let custom;
    if (role === 'examiner') {
...
function contextWindow(history, maxMessages) {
...
function chatContextWindow(history, maxMessages, voiceTurn = false) {
...
function effectiveCompletionBudget(model, maxTokens, fastChat = false) {
...
function chatPatientModel(settings) {
    const configured = settings.patientModel;
    if (!/realtime/i.test(configured))
        return configured;
    return (process.env.OPENAI_PATIENT_MODEL ||
...
async function createChatClient(settings) {
...
async function callOpenAI(messages, model, temperature, maxTokens, usageMeta) {
    const { client: openai, fallbackModel } = await createChatClient();
    const run = (activeModel, tokenBudget, fastChat = true) => openai.chat.completions.create({
        model: activeModel,
        messages,
    ...
    const record = (activeModel, response, success, error) => {
        if (!usageMeta)
            return;
        void logAiUsage({
    ...
    try {
        let response = (await run(model, maxTokens));
        let text = extractCompletionText(response);
        if (!text && model !== fallbackModel) {
            response = (await run(fallbackModel, Math.max(maxTokens, 220)));
        ...
        if (model !== fallbackModel && /realtime|gpt-5/i.test(model)) {
            const response = (await run(fallbackModel, Math.max(maxTokens, 220)));
...
async function callOpenAIStream(messages, model, temperature, maxTokens, timeoutMs, usageMeta) {
    const { client: openai, fallbackModel } = await createChatClient();
...
function logAiFallback(context, error) {
    const msg = error instanceof Error ? error.message : String(error);
    console.warn(`[AI] ${context} — OpenAI unavailable, using mock fallback: ${msg}`);
}
async function callOpenAISafe(messages, model, temperature, maxTokens, fallback, usageMeta, options) {
    const started = Date.now();
    ...
    try {
        const text = (useStream
            ...
            : timeoutMs
                ...
                : await callOpenAI(messages, model, temperature, maxTokens, usageMeta)).trim();
...
function resolvePatientLanguage(language, userMessage) {
    if (language === 'AR')
        return true;
    if (language === 'EN')
        return false;
...
function effectivePatientLanguage(language, userMessage) {
    return resolvePatientLanguage(language, userMessage) ? 'AR' : 'EN';
}
function resolveExaminerLanguage(sessionLang, studentMessage) {
    if (sessionLang === 'EN')
        return 'EN';
    if (sessionLang === 'AR')
        return 'AR';
...
function examinationExaminerLanguage() {
    return 'EN';
}
function examinerLangRule(lang) {
    return lang === 'AR'
...
function containsEnglishMedicalLeak(text) {
    return /progressive|exertional|dyspnea|tightness|shortness|breath|swelling|chest pain|months|pharmacy|training|trip/i.test(text);
}
/** Strip embedded Latin phrases that leak into Arabic patient replies (multipart collapse). */
function stripEmbeddedEnglishFromArabic(text) {
    const trimmed = text.trim();
    const arabic = (trimmed.match(/[\u0600-\u06FF]/g) || []).length;
    const latin = (trimmed.match(/[a-zA-Z]/g) || []).length;
    // Only scrub mixed Arabic+English dumps — leave pure Arabic / short Latin alone.
    if (arabic < 6 || latin < 10)
        return trimmed;
    ...
    let protectedText = trimmed.replace(/((?:عايش(?:ة)?|ساكن(?:ة)?|اتولدت|مولود(?:ة)?)\s+في\s+)([A-Za-z][A-Za-z0-9' -]*(?:,\s*[A-Za-z][A-Za-z0-9' -]*)?)(?=[.،!?؟]|$)/gi, (_match, prefix, place) => `${prefix}${protect(place.trim())}`);
    let cleaned = protectedText
        .replace(/[A-Za-z][A-Za-z0-9]*(?:[\s,',.\-/]+[A-Za-z0-9]+)+[A-Za-z0-9.]*/g, ' ')
        .replace(/\s{2,}/g, ' ')
        .replace(/\s+([،,.!?؟])/g, '$1')
        .trim();
    cleaned = cleaned.replace(/__PLACE_(\d+)__/g, (_match, index) => {
...
function finalizePatientReply(caseData, userMessage, response, lang, history = [], voiceTurn = false) {
    const multi = !voiceTurn && isMultiPartPatientQuestion(userMessage);
    const maxSentences = voiceTurn ? 2 : multi ? 12 : 5;
    if (lang === 'EN')
        return truncatePatientAnswer(response.trim(), maxSentences);
    ...
    if (!voiceTurn && (isMostlyEnglish(text) || containsEnglishMedicalLeak(text))) {
        ...
        if (asksWellbeing(userMessage))
            return patientWellbeingReply(caseData, true, false);
        if (asksAboutSymptoms(userMessage))
            return patientComplaintPhrase(caseData, true);
        if (asksName(userMessage))
            return `اسمي ${patientNameInLang(caseData, true)}.`;
...
export function unwrapExaminerPlainText(raw) {
...
function finalizeExaminerReply(text, lang) {
    const trimmed = unwrapExaminerPlainText(text);
    if (!trimmed || lang === 'EN')
        return trimmed;
    return toEgyptianColloquial(trimmed);
}
export function normalizeStudentMessage(message, sessionLang) {
...
function normalizeEgyptianColloquialInput(text) {
 ...
 */
function normalizePatientIntentText(text) {
    return ` ${text
        .normalize('NFKC')
        .toLowerCase()
        .replace(/[أإآٱ]/g, 'ا')
...
function patientNameInLang(caseData, isArabic) {
    if (!isArabic)
        return caseData.patientName;
    const lower = caseData.patientName.toLowerCase();
    if (lower.includes('tarek'))
        return 'طارق مصطفى الحداد';
    if (lower.includes('samira'))
        return 'سميرة عبد الرحمن';
    if (lower.includes('ahmed'))
        return 'أحمد';
    if (lower.includes('fatma') || lower.includes('fatima'))
        return 'فاطمة';
    if (lower.includes('mohamed') || lower.includes('mohammed'))
        return 'محمد';
    return caseData.patientName.split(' ').slice(0, 2).join(' ');
}
function patientComplaintPhrase(caseData, isArabic) {
    if (!isArabic)
        return caseData.chiefComplaint.split('.')[0].trim();
...
function formatArabicPatientDuration(value, unit) {
    if (unit === 'week') {
        if (value === 1)
            return 'أسبوع';
        if (value === 2)
    ...
    if (unit === 'day')
        return value === 1 ? 'يوم' : value === 2 ? 'يومين' : `${value} أيام`;
    if (unit === 'month')
        return value === 1 ? 'شهر' : value === 2 ? 'شهرين' : `${value} شهور`;
    return value === 1 ? 'سنة' : value === 2 ? 'سنتين' : `${value} سنين`;
}
/** Richer symptom answer for follow-ups ("بالظبط بتحس بإيه؟"). */
function patientDetailedComplaint(caseData, isArabic) {
    ...
    const symptomBits = [];
    if (/progressive|gradual|بيزيد|تدريجي/i.test(clinicalText)) {
        symptomBits.push('النهجان زاد عليا أوي');
    }
    if (/exertional|worse on exertion|with exertion|exercise|effort|مجهود/i.test(clinicalText) &&
        !/مجهود/.test(brief)) {
        symptomBits.push('وضيق النفس بيزيد مع المجهود وبيخف لما أرتاح');
    }
    if (/palpitations?|rapid heart|racing heart|flutter|tachycardia|ضربات قلبي سريعة|رفرفة في صدري|ضربات قلبي سريعه|رفرفه/i.test(clinicalText)) {
        symptomBits.push('وضربات قلبي سريعة وبحس برفرفة كده');
    }
    if (/paroxysmal nocturnal dyspnea|\bpnd\b|wakes? gasping|wakes?.*breath|بصحى.*مخنوق/i.test(clinicalText)) {
    ...
    else if (/orthopnea|worse when lying flat|lying flat|لما أنام|على ضهري/i.test(clinicalText)) {
        symptomBits.push('والنفس بيضايقني أكتر لما أنام على ضهري');
    }
    if (/lightheaded|dizz|blurred vision|دوخة|زغللة/i.test(clinicalText)) {
        symptomBits.push('وبيجيلي دوخة أو زغللة خصوصًا مع المجهود');
    }
    if (/productive cough|cough.*sputum|بلغم|كحة.*بلغم/i.test(clinicalText)) {
        symptomBits.push('وعندي كحة ببلغم');
    }
    else if (/\bcough\b|كحة|سعال/i.test(clinicalText)) {
        symptomBits.push('وعندي كحة');
    }
    const feverExplicitlyDenied = /(?:no|denies?|without)[^.]{0,50}(?:fever|pyrexia)|(?:fever|pyrexia)[^.]{0,30}(?:absent|denied)/i.test(clinicalText);
    if (!feverExplicitlyDenied &&
        /high[- ]grade fever|(?<!rheumatic\s)fever|سخونية|حرارة/i.test(clinicalText)) {
        symptomBits.push('وكان عندي سخونية قريب');
    }
    if (/ankle swelling|leg swelling|pitting edema|oedema|edema|تورم.*رجل/i.test(clinicalText)) {
    ...
    if (/tonsill|لوز/i.test(histLower))
        extraBits.push('وقبل كده كان عندي مشاكل في اللوز');
    if (/hypertens|ضغط/i.test(histLower))
        extraBits.push('وعندي ضغط');
    if (/diabet|سكر/i.test(histLower))
        extraBits.push('وعندي سكر');
...
function asksSymptomDetails(text) {
    return /بالظبط|بالضبط|بتحسي|بتحس|تحسي|تحس|تصف|وصف|يعني\s*(?:إيه|ايه)|إيه\s*هي|ايه\s*هي|إيه\s*عبار|ايه\s*عبار|التفاصيل|[اأإ]كتر|أكثر|وض[ّ']?ح|صف.?لي|what exactly|describe|tell me more|more detail|what do you feel|how does it feel/i.test(text);
}
...
function isNearDuplicatePatientReply(candidate, history) {
...
function isVagueStudentMessage(text) {
    const t = text.trim();
    return /^(أ?ييه|ايه|إيه|اي|eh|eih|أيه)\s*[؟?.!]*$/i.test(t) || t.length < 3;
}
function isEmpathyOrBlessing(text) {
    const t = text.trim();
    return /الف\s*(مليون\s*)?سلام|مليون\s*سلام|سلام[ةه]\s*(عليك|عليكي)?|سلامتك|ربنا\s*يخليك|ربنا\s*يشفيك|الله\s*يسلمك|get\s*well|bless\s*you/i.test(t);
}
function isGreetingOnly(text) {
    ...
    if (/^(السلام عليكم|سلام عليكم)(\s+ورحمة\s*الله(\s+وبركاته)?)?(\s+(يا\s*)?دكتور)?$/i.test(t)) {
        return true;
    }
    if (/^(صباح الخير|مساء الخير|good morning|good afternoon|good evening|peace be upon you)(\s+(يا\s*)?دكتور|\s+doctor)?$/i.test(t)) {
        return true;
    }
    const core = t.replace(/\s*(يا\s+)?دكتور\s*$/i, '').trim() || t;
    const words = core.split(' ').filter(Boolean);
    const wordGreeting = /^(أ?هلا|اهلا|مرحبا?|مرحب|سلام|السلام|عليكم|ورحمة|الله|وبركاته|هاي|hi|hello|hey|هيلو|هالو|حيلو|هلو|هلا)$/i;
...
function quickSocialPatientReply(caseData, userMessage, lang, history, voiceTurn) {
    ...
    if (asksWellbeing(userMessage)) {
        ...
        const hasOtherFacts = intents.some((intent) => intent !== 'wellbeing' &&
            intent !== 'greeting' &&
            intent !== 'empathy');
        if (hasOtherFacts)
            return null;
...
function pickArabicCaseText(...candidates) {
...
function patientScenarioSnippet(caseData, isArabic) {
    const scenario = caseData.scenarioPrompt?.trim() ?? '';
    const chief = caseData.chiefComplaint?.trim() ?? '';
    ...
    if (isArabic) {
        ...
        if (arabicBlock)
            return firstSentences(arabicBlock, 2);
    ...
    return (firstSentences(scenario, 2) ||
        firstSentences(chief, 2) ||
        patientComplaintPhrase(caseData, false));
}
function patientRichComplaint(caseData, isArabic) {
    if (!isArabic) {
    ...
    if (/^والله/i.test(narrative))
        return narrative;
    return `والله يا دكتور ${narrative}`;
}
function patientWellbeingReply(caseData, isArabic, voiceTurn) {
    if (!isArabic)
        return voiceTurn ? 'Not great, doctor.' : 'Honestly doctor, I have not been feeling well lately.';
    if (voiceTurn)
        return 'مش في أحسن حالي دكتور.';
...
function patientIsFemale(caseData) {
    return /^(?:f|female|أنثى)/i.test(caseData.patientGender.trim());
}
function patientGreetingOnlyReply(caseData, isArabic, userMessage = '') {
    const msg = userMessage.trim();
    if (!isArabic) {
        if (/peace|salam|assalam/i.test(msg))
            return 'Peace be upon you too, doctor.';
        return 'Hello doctor.';
    }
    if (/السلام|سلام عليكم/i.test(msg))
        return 'وعليكم السلام يا دكتور.';
    if (/صباح/i.test(msg))
        return 'صباح النور يا دكتور.';
    if (/مساء/i.test(msg))
        return 'مساء النور يا دكتور.';
    return 'أهلاً يا دكتور.';
}
function patientOpeningReply(caseData, isArabic) {
...
function patientEmpathyReply(caseData, isArabic) {
    if (!isArabic) {
...
function truncatePatientAnswer(text, maxSentences = 2) {
    const cleaned = text.trim();
    if (!cleaned)
        return cleaned;
    const parts = cleaned.split(/(?<=[.!?؟])\s+/).filter(Boolean);
...
function enforcePatientLanguage(text, isArabic) {
    const trimmed = text.trim();
    if (!trimmed || !isArabic)
        return trimmed;
...
function isDoctorIntroduction(text) {
    return /^(good\s+(morning|afternoon|evening)|nice to meet|pleased to meet|i'?m\s+(dr|doctor)|my name is.*(dr|doctor)|أنا\s+(د|دكتور)|اسمي\s+(د|دكتور)|تشرفنا|نورت)/i.test(text.trim());
}
function asksAboutSymptoms(text) {
    if (isVagueStudentMessage(text))
        return false;
    ...
    return /why|what brought|what brings|present|complain|symptom|problem|chief|feel|wrong|happening|issue|breath|dyspnea|swell|pain|chest|tell me about|describe|history of|ليه|سبب|شكو|شكوى|شكواك|شكوتك|شكوايتك|بتشتكي|بتشكو|تشتكي|اشتكي|اشتكيت|بتشكو|احكي(?:لي)?|عرض|وجع|ألم|الم|ضيق|تنفس|تورم|حاس|حاسس|بتعاني|تعاني من|مشكل|إيه اللي جابك|إيه الحاجة|الحاجة اللي|إيه المشكلة|إيه مشكل|إيه جابك|جابك هنا|جيت ليه|ليه جيت|جاي.*ليه|تعبك|تعبان.*إيه|وديجتي|ودجتي|وش جيت|عندك إيه|عندك ايه|إيه اللي عندك|ما الذي|شكو.*من|بتشتكي\s*من|تشتكي\s*من|what.*wrong|what.*problem|what.*matter|what.*complain|الحكاية|القصة|بدأت|اتطورت|ازاي\s*بدأت/i.test(text);
}
function asksSmokingOrAlcohol(text) {
    return /دخن|تدخين|تدخن|سجاير|سجائر|تشرب|بتشرب|ليك\s*في\s*(?:الشرب|السجاير)|خمر|كحول|حشيش|smoke|smok|alcohol|drink|drinking|cigarette/i.test(text);
}
function asksBirthPlace(text) {
    return /اتولد|اتولدت|اتولدتي|مولود|مكان\s*الميلاد|where\s+(?:were|are)\s+you\s+born|birth\s*place|born\s+where/i.test(text);
}
function asksHobbiesOrSport(text) {
    ...
    if (asksSmokingOrAlcohol(text) && !hasExplicitCasualTopic)
        return false;
    return hasExplicitCasualTopic || /(?:بتحب|تحب)/i.test(text);
}
function asksName(text) {
    return /(?:name|your name|اسم|اسمك|who are you|مين\s*(?:اسمك|حضرتك)|من انت|may i have your name|what is your name|اسمك\s*إ?يه|اسمك\s*ايه|اسم حضرتك)/i.test(text);
}
function asksAge(text) {
    return /(?:how\s*old|years?\s*old|your\s*age)|(?:عمرك|سنك|(?:كم|كام)\s*عمر|(?:كم|كام)\s*سنة|(?:كم|كام)\s*سن|عندك\s*(?:كم|كام)\s*سنة|عندك\s*(?:كم|كام)\s*سن)/i.test(text);
}
function asksPriorDoctorVisit(text) {
    return /روحت.*دكتور|رحت.*دكتور|زرت.*دكتور|دكتور قبل|مستشفى قبل|seen a doctor|doctor before|visited.*doctor|hospital before|previous doctor/i.test(text);
}
function asksResidence(text) {
    return /ساكن فين|ساكنة فين|عايش فين|عايشة فين|مقيم فين|ساكن في|منين|من فين|where.*live|where do you live|your address|بتسكن/i.test(text);
}
function asksOccupation(text) {
    return /(?:occupation|job|work|what do you do|your work)|(?:شغلك|شغلك\s*(?:إيه|ايه)|شغال(?:ة)?(?:\s*(?:إيه|ايه))?|بتشتغل|بتشتغلي|بتعمل\s*(?:إيه|ايه)|مهنتك|وظيفتك|بتاع\s*(?:إيه|ايه))/i.test(text);
}
function asksWeight(text) {
    return /(?:how\s*much\s*do\s*you\s*weigh|your\s*weight|what.*weight|how\s*heavy)|(?:وزنك|وزن حضرتك|بتوزن\s*كام|تزن\s*كام|كام كيلو|كم كيلو)/i.test(text);
}
function asksHeight(text) {
    return /(?:how\s*tall|your\s*height|what.*height)|(?:طولك|طول حضرتك|كام سم|كم سم)/i.test(text);
}
/** Parse a stored vitalSigns JSON/string for a labelled measurement (weight/height). */
function readVitalMeasurement(caseData, keys) {
...
function patientWeightPhrase(caseData, isArabic) {
...
function patientHeightPhrase(caseData, isArabic) {
...
function patientPriorDoctorPhrase(caseData, isArabic) {
...
function patientResidencePhrase(caseData, isArabic, naturalWrittenArabic = false) {
    ...
    const liveMatch = explicitCaseText(caseData).match(/(?:lives?|living|resides?|resident)\s+in\s+([^.;,\n]+)|(?:عايش(?:ة)?|ساكن(?:ة)?|مقيم(?:ة)?)\s+(?:في|ب)\s*([^.;،\n]+)/i);
    if (liveMatch) {
        return isArabic
...
function explicitCaseText(caseData) {
    return [
...
function arabicCasePlaceName(place) {
    ...
    if (/^shobra\s+(?:al-|el-)?kheima\s*,\s*cairo$/.test(normalized))
        return 'شبرا الخيمة، القاهرة';
    if (/^shobra\s+(?:al-|el-)?kheima$/.test(normalized))
        return 'شبرا الخيمة';
    return place.trim();
}
function patientOccupationPhrase(caseData, isArabic) {
    const social = [
    ...
    const cleanOccupation = (value) => value.replace(/\s*\([^)]*\)\s*/g, ' ').replace(/\s{2,}/g, ' ').trim();
    const arabicOccupation = (value) => {
        const raw = cleanOccupation(value);
        const lower = raw.toLowerCase();
        if (/house\s*painter|painter/.test(lower))
            return 'نقاش';
        if (/teacher/.test(lower))
            return female ? 'مدرسة' : 'مدرس';
        if (/student/.test(lower))
            return female ? 'طالبة' : 'طالب';
        if (/doctor|physician/.test(lower))
            return female ? 'طبيبة' : 'طبيب';
        if (/nurse/.test(lower))
            return female ? 'ممرضة' : 'ممرض';
        if (/pharmacist/.test(lower))
            return female ? 'صيدلانية' : 'صيدلي';
        if (/engineer/.test(lower))
            return female ? 'مهندسة' : 'مهندس';
        if (/accountant/.test(lower))
            return female ? 'محاسبة' : 'محاسب';
        if (/driver/.test(lower))
            return 'سواق';
        if (/worker|laborer/.test(lower))
            return female ? 'عاملة' : 'عامل';
    ...
    if (apprenticeMatch) {
        ...
        if (!isArabic)
            return `I work as an apprentice ${raw}.`;
        return `بشتغل ${arabicOccupation(raw)} تحت التدريب.`;
    }
    const occMatch = social.match(/occupation\s*:\s*([^.;\n]+)/i) ||
...
function asksAllergy(text) {
    return /allerg|حساس|حساسية/i.test(text);
}
function asksOnsetDuration(text) {
...
function asksExertionalDyspnea(text) {
...
function asksOrthopneaPnd(text) {
...
function combinedCaseClinicalText(caseData) {
    return [
...
function patientOnsetDurationPhrase(caseData, isArabic) {
    const chief = caseData.chiefComplaint || '';
    const history = caseData.medicalHistory || '';
    const source = `${chief} ${history}`;
    const match = chief.match(/(?:for\s+)?(?:about\s+|around\s+|approximately\s+)?(\d+)\s*-?\s*(day|week|month|year)s?(?:\s+duration)?/i) ||
    ...
    if (!isArabic)
        return `It started about ${n} ${unit}${n === 1 ? '' : 's'} ago.`;
    return `بدأت من حوالي ${formatArabicPatientDuration(n, unit)}.`;
}
function patientExertionalDyspneaPhrase(caseData, isArabic) {
...
function patientDizzinessPhrase(caseData, isArabic) {
    const text = combinedCaseClinicalText(caseData);
    ...
    if (denied)
        return isArabic ? 'لا، مش بيجيلي دوخة أو زغللة.' : 'No, I do not get dizziness or blurred vision.';
    if (present) {
...
function patientCoughPhrase(caseData, isArabic) {
    const text = combinedCaseClinicalText(caseData);
    ...
    if (denied)
        return isArabic ? 'لا، مفيش كحة عندي.' : 'No, I do not have a cough.';
    if (productive) {
    ...
    if (cough)
        return isArabic ? 'آه، عندي كحة.' : 'Yes, I have a cough.';
    return patientUncertaintyPhrase(caseData, isArabic, isArabic ? 'الكحة أو البلغم' : 'a cough or sputum');
}
function patientRecentFeverPhrase(caseData, isArabic) {
    const text = combinedCaseClinicalText(caseData);
    ...
    if (denied)
        return isArabic ? 'لا، مكنش عندي سخونية قريب.' : 'No, I have not had a recent fever.';
    if (present)
        return isArabic ? 'آه، كان عندي سخونية عالية من قريب.' : 'Yes, I had a high fever recently.';
    return patientUncertaintyPhrase(caseData, isArabic, isArabic ? 'السخونية الأخيرة' : 'a recent fever');
}
function explicitPillowCount(caseData) {
    const text = combinedCaseClinicalText(caseData);
    const match = text.match(/(?:sleeps?\s+on|uses?)\s+(\d+)\s+pillows?/i) ||
        text.match(/(?:ينام|بنام|تنام)\s+على\s*([0-9٠-٩]+)\s*مخد/i);
    return match?.[1] ?? null;
}
function patientPillowCountPhrase(caseData, isArabic) {
...
function patientOrthopneaPndPhrase(caseData, isArabic, userMessage = '') {
    const text = combinedCaseClinicalText(caseData);
    const lower = text.toLowerCase();
    const question = normalizePatientIntentText(userMessage);
    const asksPillows = /pillows?|مخد(?:ة|ه|ات|تين)?|كام\s*مخد|كم\s*مخد/i.test(question);
    const asksPnd = /\bpnd\b|paroxysmal nocturnal|wakes?.*(?:gasp|breath)|بتصح[ىي].*(?:مخنوق|نفس)|تصح[ىي].*(?:مخنوق|نفس)|النفس\s*بيصح[يى]ك\s*من\s*النوم|بتقوم\s*بالليل\s*عشان\s*مش\s*قادر(?:ة)?\s*تتنفس/i.test(question);
    const asksOrthopnea = /orthopnea|lying flat|lie flat|نايم.*(?:مفرود|مسطح)|لما\s*تنام|النوم.*(?:ضيق|نفس)/i.test(question);
    const pillows = explicitPillowCount(caseData);
    const hours = text.match(/after\s*~?\s*(\d+)\s+hours?/i);
    const hasPnd = /paroxysmal nocturnal dyspnea|\bpnd\b|wakes? gasping|wakes?.*breath/i.test(lower);
    const hasOrthopnea = /orthopnea|worse when lying flat|worse lying flat|lying flat/i.test(lower);
    const deniesPnd = /denies?[^.]{0,100}(?:pnd|paroxysmal nocturnal dyspnea)|no\s+(?:pnd|paroxysmal nocturnal dyspnea)/i.test(text);
    const deniesOrthopnea = /denies?[^.]{0,100}orthopnea|no\s+orthopnea/i.test(text);
...
function patientEdemaPhrase(caseData, isArabic) {
    const text = combinedCaseClinicalText(caseData);
...
function patientMedicationPhrase(caseData, isArabic) {
    ...
    if (/ventolin|salbutamol/i.test(lower))
        parts.push('وبستخدم بخاخ فنتولين للربو');
    if (/home oxygen|oxygen/i.test(lower))
        parts.push('وأوقات بستخدم أكسجين في البيت وقت النوبات');
    ...
    if (parts.length > 0)
        return `${parts.join('، ')}.`;
    if (/[^\x00-\x7F]/.test(meds))
        return firstSentences(meds, 2);
    return `آه، باخد أدوية بانتظام.`;
}
function patientHypertensionPhrase(caseData, isArabic) {
    const history = caseData.medicalHistory || '';
...
function patientDiabetesPhrase(caseData, isArabic) {
    const history = caseData.medicalHistory || '';
 ...
 */
function semanticEgyptianPatientIntent(question) {
    ...
    if (has(/(?:^|\s)(?:متجوز|متزوج|اعزب|سنجل)(?:\s|$)/))
        return 'marital';
    ...
    if (has(/(?:^|\s)(?:بتاخد|بتستعمل|ادوية|دواء|دوا)(?:\s|$)/))
        return 'medication';
    const wakeBreathPhrase = has(/(?:^|\s)(?:بتصحي|تصحي|بتقوم)(?:\s|$)/) &&
...
function messageQuestionParts(message) {
    const trimmed = message.trim();
    ...
    const coordinatedParts = trimmed
        ...
        .map((s) => s.trim())
        .filter((s) => s.length > 2);
    if (coordinatedParts.length >= 3)
        return coordinatedParts;
    const inlineMarkers = [
...
function intentForQuestionPart(part) {
    ...
    if (isEmpathyOrBlessing(part))
        return 'empathy';
    // Specific clinical intents MUST be checked before the broad symptom detector.
    if (asksPillowCount(part))
        return 'pillowCount';
    if (asksOrthopneaPnd(part))
        return 'orthopneaPnd';
    if (asksDizzinessOrBlurredVision(part))
        return 'dizziness';
    if (asksCoughOrSputum(part))
        return 'cough';
    if (asksRecentFever(part))
        return 'recentFever';
    if (asksEdema(part))
        return 'edema';
    if (asksAllergy(part))
        return 'allergy';
    if (asksMedication(part))
        return 'medication';
    if (asksSmokingOrAlcohol(part))
        return 'socialHabits';
    if (asksFamilyHistory(part))
        return 'familyHistory';
    if (asksHypertension(part))
        return 'hypertension';
    if (asksDiabetes(part))
        return 'diabetes';
    if (asksOnsetDuration(part))
        return 'onsetDuration';
    if (asksExertionalDyspnea(part))
        return 'exertionalDyspnea';
    if (asksAboutSymptoms(part))
        return 'symptoms';
    // Check preferences before name: "مين لاعبك المفضل؟" is not asking the
    // patient's name merely because it begins with "مين".
    if (asksHobbiesOrSport(part))
        return 'hobbies';
    if (asksName(part))
        return 'name';
    if (asksAge(part))
        return 'age';
    if (asksBirthPlace(part))
        return 'birthPlace';
    if (asksResidence(part))
        return 'residence';
    if (asksOccupation(part))
        return 'occupation';
    if (asksWeight(part))
        return 'weight';
    if (asksHeight(part))
        return 'height';
    if (asksPriorDoctorVisit(part))
        return 'priorDoctor';
    if (asksNationality(part))
        return 'nationality';
    if (asksMaritalStatus(part))
        return 'marital';
    if (asksGender(part))
        return 'gender';
    if (asksWellbeing(part))
        return 'wellbeing';
    if (isGreetingOnly(part) || isDoctorIntroduction(part))
        return 'greeting';
    return null;
}
function resolvePatientQuestionIntents(message) {
    ...
    const specificClinicalIntents = new Set([
        'onsetDuration',
        'exertionalDyspnea',
        'pillowCount',
        'orthopneaPnd',
    ...
    for (const [intent, detect] of detectors) {
        if (intent === 'symptoms' && intents.some((item) => specificClinicalIntents.has(item)))
            continue;
...
function isMultiPartPatientQuestion(message) {
    const intents = resolvePatientQuestionIntents(message);
    if (intents.length >= 2)
        return true;
    return messageQuestionParts(message).length >= 2;
}
function resolvePrimaryPatientQuestionIntent(message) {
    const intents = resolvePatientQuestionIntents(message);
    return intents.length > 0 ? intents[intents.length - 1] : null;
}
function asksFamilyHistory(text) {
    return /(family history|family.*(history|disease|problem)|تاريخ.*(عائلي|عيلة)|العيلة|عيلتك|أهل.*(مرض|زي|نفس)|history of.*family)/i.test(text);
}
function patientFamilyHistoryPhrase(caseData, isArabic, userMessage = '') {
    ...
    if (asksSameProblem) {
        if (noSimilar)
            return isArabic ? 'مفيش حد قريب في العيلة عنده نفس المشكلة.' : 'No close family member has the same problem.';
        if (sameProblemDocumented)
            return isArabic ? 'آه، في حد في العيلة عنده نفس المشكلة.' : 'Yes, someone in my family has the same problem.';
    ...
    if (isArabic) {
        if (noSimilar)
            return 'مفيش حد قريب في العيلة عنده نفس المشكلة.';
        if (/hypertension|high blood pressure|ضغط/i.test(family))
            return 'في حد في العيلة عنده ضغط.';
        return `في تاريخ مرضي في العيلة.`;
    }
    return firstSentences(family, 1);
}
function patientSocialHabitsPhrase(caseData, isArabic, userMessage = '', naturalWrittenArabic = false) {
    ...
    const currentSmoker = !formerSmoker &&
        !neverSmoker &&
    ...
    const alcoholExplicitYes = !alcoholExplicitNo &&
    ...
    if (asksSmoking && !asksAlcohol)
        return smokingReply;
    if (asksAlcohol && !asksSmoking)
        return alcoholReply;
    if (asksSmoking && asksAlcohol)
        return isArabic
...
function patientBirthPlacePhrase(caseData, isArabic, naturalWrittenArabic = false) {
...
function patientHobbiesPhrase(caseData, isArabic, userMessage = '') {
    ...
    if (/football|soccer|كره|كرة/i.test(lowerQuestion)) {
        if (/football|soccer|كره|كرة/i.test(lowerProfile))
    ...
    if (/hobby|hobbies|هواي|interest|رياضة|sport|وقت\s*فراغ|free\s*time/.test(lowerQuestion)) {
        if (/sport|رياض/i.test(lowerProfile))
            return isArabic ? 'بحب الرياضة.' : 'I like sports.';
        return isArabic ? 'معنديش هواية مفضلة معينة.' : 'I do not have a particular hobby.';
    }
    return isArabic ? 'معنديش تفضيل معين في الموضوع ده بصراحة.' : 'I do not have a particular preference.';
}
function deterministicReplyForIntent(caseData, intent, isArabic, voiceTurn = true, userMessage = '', history = []) {
    ...
    switch (intent) {
        ...
        case 'marital': {
            const social = (caseData.patientMaritalStatus?.trim() ||
                explicitCaseText(caseData) ||
                '').toLowerCase();
        ...
        case 'symptoms': {
            ...
            if (asksSymptomDetails(userMessage)) {
                ...
                if (!isNearDuplicatePatientReply(detailed, history))
                    return detailed;
                return null;
            }
...
function formatArabicPillows(countStr) {
    const num = parseInt(countStr, 10);
    if (isNaN(num))
        return `${countStr} مخدات`;
    if (num === 1)
        return 'مخدة واحدة';
    if (num === 2)
        return 'مخدتين';
...
function buildCohesiveArabicDemographics(caseData, intents, userMessage) {
    const feminine = patientIsFemale(caseData);
    const parts = [];
    // 1. Greeting
    ...
    if (intents.includes('symptoms')) {
        ...
        if (comp)
            parts.push(`وبالنسبة لسبب زيارتي، ${comp}`);
    }
    let result = '';
...
function deterministicReplyForIntents(caseData, intents, isArabic, voiceTurn = true, userMessage = '', history = []) {
    if (intents.length === 0)
        return null;
    if (isArabic) {
    ...
    if (intents.length === 1) {
        return deterministicReplyForIntent(caseData, intents[0], isArabic, voiceTurn, userMessage, history);
    }
    const replies = intents
        .map((intent) => deterministicReplyForIntent(caseData, intent, isArabic, voiceTurn, userMessage, history))
...
function sanitizePatientResponse(caseData, userMessage, response, language, voiceTurn = false) {
    ...
    if ((intent === 'greeting' || isGreetingOnly(userMessage)) &&
        ...
        const dumpedComplaint = complaintSnippet.length > 8 && responseLower.includes(complaintSnippet.slice(0, 12));
        const dumpedMedical = /حمى|سخونية|قشعريرة|صداع|fever|chills|headache|ألم|وجع|ضيق|تنفس|تورم/i.test(text);
...
function getDeterministicPatientResponse(caseData, userMessage, language, history = [], voiceTurn = false) {
    const isArabic = resolvePatientLanguage(language, userMessage);
    const text = userMessage.trim().toLowerCase();
    const intents = resolvePatientQuestionIntents(userMessage);
    if (intents.length > 0) {
        const intentReply = deterministicReplyForIntents(caseData, intents, isArabic, voiceTurn, userMessage, history);
        if (intentReply) {
            ...
            if (isNearDuplicatePatientReply(intentReply, history)) {
                ...
                if (intents.includes('symptoms')) {
                    ...
                    if (asksSymptomDetails(userMessage)) {
                        const detailed = patientDetailedComplaint(caseData, isArabic);
                        if (detailed && !isNearDuplicatePatientReply(detailed, history))
                            return detailed;
                        const rich = patientRichComplaint(caseData, isArabic);
                        if (rich && !isNearDuplicatePatientReply(rich, history))
                            return rich;
                    }
                    return intentReply;
                }
    ...
    if (asksAboutSymptoms(text)) {
        const reply = asksSymptomDetails(text) ||
        ...
        if (isNearDuplicatePatientReply(reply, history))
            return null;
        return reply;
    }
    return null;
}
function mockPatientResponse(caseData, userMessage, language, history = []) {
    const isArabic = resolvePatientLanguage(language, userMessage);
    const studentTurn = history.filter((m) => m.role === 'STUDENT').length;
    if (isEmpathyOrBlessing(userMessage)) {
        return patientEmpathyReply(caseData, isArabic);
    ...
    if (isGreetingOnly(userMessage) || isDoctorIntroduction(userMessage)) {
        if (studentTurn === 0)
            return patientOpeningReply(caseData, isArabic);
...
function buildZeroParticipationEvaluation(caseData, messages, lang = 'EN') {
    const caseTitle = lang === 'AR' ? caseData.titleAr || caseData.titleEn : caseData.titleEn;
    ...
    const noAttempt = pickLang('No attempt: the station was ended without any interaction with the patient or examiner.', 'لا توجد محاولة: تم إنهاء المحطة دون أي تفاعل مع المريض أو الممتحن.', lang);
    return {
        totalScore: 0,
        ...
        organizationScore: 0,
        closingScore: 0,
        strengths: pickLang('None recorded — no part of the station was attempted.', 'لا يوجد — لم تتم محاولة أي جزء من المحطة.', lang),
        weaknesses: noAttempt,
        missedQuestions: pickLang('The entire station was missed: history, examination, diagnosis, and management were not attempted.', 'فاتت المحطة بالكامل: لم تتم محاولة التاريخ أو الفحص أو التشخيص أو الإدارة.', lang),
        clinicalErrors: pickLang('Not applicable — no clinical actions were taken.', 'لا ينطبق — لم تُتخذ أي إجراءات سريرية.', lang),
        recommendations: pickLang('Attempt the station: introduce yourself, take a focused history, perform the examination, then give a diagnosis and management plan.', 'حاول إكمال المحطة: قدّم نفسك، خذ تاريخاً مركزاً، أجرِ الفحص، ثم قدّم التشخيص وخطة الإدارة.', lang),
        idealApproach: pickLang(`Introduce yourself, confirm identity, explore ${caseData.chiefComplaint} using SOCRATES with exertional symptoms, ask about rheumatic fever and penicillin prophylaxis, complete examination with murmur description, then state ${caseData.finalDiagnosis} with echo referral and management plan.`, `قدّم نفسك، تأكد من الهوية، استكشف ${caseData.chiefComplaint} بـ SOCRATES مع أعراض المجهود، اسأل عن الحمى الروماتيزمية والبروفيلاكس بالبنسلين، أكمل الفحص مع وصف الـ murmur، ثم اذكر ${caseData.finalDiagnosis} مع إحالة echo وخطة إدارة.`, lang),
        fullReport: `## ${reportTitle}\n\n**${caseLabel}:** ${caseTitle}\n**${overallLabel}:** 0/100\n\n> ${noAttempt}\n\n### ${perfSummary}\n- ${commLabel}: 0/100\n- ${histLabel}: 0/100\n- ${reasonLabel}: 0/100\n- ${orgLabel}: 0/100\n- ${closeLabel}: 0/100`,
    };
}
function mockExaminerEvaluation(caseData, messages, lang = 'EN', context = {}) {
    ...
    const historyMsgCount = studentMessages.filter((m) => m.stage === 'history' || m.stage === 'history:examiner').length;
    ...
    const creditedExaminationMessages = studentMessages.filter((m) => {
        if (!m.stage.startsWith('examination:'))
            return false;
        const answer = m.content.trim();
        if (!answer)
            return false;
        if (studentGaveUpAnswer(answer))
            return false;
        return true;
    });
    const creditedManeuverStages = new Set(creditedExaminationMessages.map((m) => m.stage));
    ...
    const demonstratedExamComponentCount = [...creditedManeuverStages].reduce((count, stage) => {
        const maneuverId = stage.slice('examination:'.length);
        const expected = physicalExamForScoring[maneuverId]?.trim() || '';
    ...
    const meaningfulStages = new Set(meaningfulNonExamStages.map((m) => m.stage));
    for (const stage of creditedManeuverStages) {
        meaningfulStages.add(stage);
    }
    const stagesCovered = meaningfulStages.size;
    const checklistHits = [
        hasPattern(studentText, /\b(introduc|my name is|i am dr|i'm dr|أنا دكتور|انا دكتور|أنا الدكتور|انا الدكتور|مرحباً أنا|مرحبا انا)/i),
    ...
    const diagnosisText = studentMessages
        ...
        .join(' ')
        .toLowerCase();
    const askedIntroduction = hasPattern(studentText, /\b(introduc|my name is|i am dr|i'm dr|أنا دكتور|انا دكتور|أنا الدكتور|انا الدكتور|مرحباً أنا|مرحبا انا)/i);
    ...
    const correctDiagnosis = hasPattern(diagnosisText, /\b(aortic stenosis|mitral regurg|rheumatic|as\s*\+\s*mr|combined.*stenosis|تضيق.*أورط|قصور.*mitral|rheumatic heart)/i);
    ...
    let historyTakingScore = 15 +
        Math.min(20, historyMsgCount * 5) +
        checklistHits * 6;
    if (askedComplaint)
        historyTakingScore += 12;
    if (askedExertional)
        historyTakingScore += 8;
    if (askedRheumatic)
        historyTakingScore += 12;
    if (askedMeds)
        historyTakingScore += 8;
    if (askedFamily)
        historyTakingScore += 6;
    if (askedRedFlags)
        historyTakingScore += 10;
    historyTakingScore = Math.min(100, historyTakingScore);
    let clinicalReasonScore = 10 +
        creditedManeuverCount * 12 +
        Math.min(20, demonstratedExamComponentCount * 8) +
        Math.min(15, diagnosisMsgCount * 10);
    if (describedExam)
        clinicalReasonScore += 15;
    if (correctDiagnosis)
        clinicalReasonScore += 35;
    else if (partialDiagnosis)
        clinicalReasonScore += 16;
    if (gaveManagement)
        clinicalReasonScore += 12;
    clinicalReasonScore = Math.min(100, clinicalReasonScore);
    const organizationScore = Math.min(100, 18 +
        stagesCovered * 10 +
        Math.min(20, historyMsgCount * 4) +
        (askedIntroduction ? 12 : 0) +
        (askedComplaint ? 10 : 0) +
        creditedManeuverCount * 6);
    const closingScore = Math.min(100, 12 +
    ...
    const strengths = [];
    if (askedIntroduction) {
        strengths.push(pickLang('Opened with a professional introduction.', 'بدأ بمقدمة مهنية واضحة.', lang));
    }
    if (askedComplaint) {
        strengths.push(pickLang('Explored the presenting complaint.', 'استكشف الشكوى الرئيسية.', lang));
    }
    if (askedRheumatic) {
        strengths.push(pickLang('Asked about rheumatic fever history — key for this case.', 'سأل عن تاريخ الحمى الروماتيزمية — مهم جداً في الحالة دي.', lang));
    }
    if (askedExertional) {
        strengths.push(pickLang('Explored exertional symptoms appropriately.', 'استكشف أعراض المجهود بشكل مناسب.', lang));
    }
    if (describedExam) {
        strengths.push(pickLang('Provided examination findings during the viva.', 'قدّم ملاحظات فحص سريري أثناء الـ viva.', lang));
    }
    if (correctDiagnosis) {
        strengths.push(pickLang(`Correctly identified the diagnosis (${caseData.finalDiagnosis}).`, `حدّد التشخيص بشكل صحيح (${caseData.finalDiagnosis}).`, lang));
    }
    if (strengths.length === 0) {
        strengths.push(pickLang('Engaged with the simulated patient and attempted the station.', 'تفاعل مع المريض المحاكى وحاول إكمال المحطة.', lang));
    }
    const weaknesses = [];
    if (!askedIntroduction) {
        weaknesses.push(pickLang('Did not clearly introduce themselves or establish rapport.', 'لم يقدّم نفسه بوضوح أو يبني rapport مع المريض.', lang));
    }
    if (!askedRheumatic) {
        weaknesses.push(pickLang('Did not explore history of rheumatic fever — critical for this case.', 'لم يسأل عن تاريخ الحمى الروماتيزمية — نقطة حاسمة في الحالة دي.', lang));
    }
    if (!askedMeds) {
        weaknesses.push(pickLang('Insufficient exploration of medication history / penicillin prophylaxis.', 'استكشاف غير كافٍ لتاريخ الأدوية / البروفيلاكس بالبنسلين.', lang));
    }
    if (!askedRedFlags) {
        weaknesses.push(pickLang('Missed important red-flag screening questions.', 'فاتته أسئلة مهمّة عن red flags.', lang));
    }
    if (!describedExam) {
        weaknesses.push(pickLang('Limited or no structured examination findings documented.', 'ملاحظات الفحص السريري محدودة أو غير منظمة.', lang));
    }
    if (!correctDiagnosis && !partialDiagnosis) {
        weaknesses.push(pickLang('Final diagnosis was absent or not clinically supported.', 'التشخيص النهائي غائب أو غير مدعوم سريرياً.', lang));
    }
    if (weaknesses.length === 0) {
        weaknesses.push(pickLang('Minor refinements needed in systematic coverage and closing summary.', 'محتاج تحسينات بسيطة في التغطية المنهجية والملخص الختامي.', lang));
    }
    const missed = [];
    if (!askedRheumatic) {
        missed.push(pickLang('History of rheumatic fever and penicillin prophylaxis compliance', 'تاريخ الحمى الروماتيزمية والالتزام بالبروفيلاكس بالبنسلين', lang));
    }
    if (!askedExertional) {
        missed.push(pickLang('Exertional dyspnea progression and functional limitation', 'تطور ضيق النفس مع المجهود والقصور الوظيفي', lang));
    }
    if (!askedRedFlags) {
        missed.push(pickLang('Syncope, palpitations, orthopnea/PND', 'الإغماء، الخفقان، orthopnea/PND', lang));
    }
    if (!askedFamily) {
        missed.push(pickLang('Relevant family cardiac history', 'التاريخ العائلي القلبي relevant', lang));
    }
    if (missed.length === 0) {
        missed.push(pickLang('Consider deeper exploration of differential diagnoses and investigation planning.', 'فكّر في استكشاف أعمق للتشخيصات التفاضلية وخطة التحاليل.', lang));
    }
    const errors = [];
    if (!correctDiagnosis && diagnosisText.length > 0) {
        errors.push(pickLang(`Submitted diagnosis did not match the expected: ${caseData.finalDiagnosis}.`, `التشخيص المقدّم لا يطابق المتوقع: ${caseData.finalDiagnosis}.`, lang));
    }
    if (!askedMeds && /penicillin|prophylaxis/i.test(caseData.medicationHistory)) {
        errors.push(pickLang('Did not assess penicillin prophylaxis adherence despite it being relevant to the case.', 'لم يقيّم الالتزام بالبروفيلاكس بالبنسلين رغم أهميته في الحالة.', lang));
    }
    if (errors.length === 0) {
        errors.push(pickLang('No major clinical safety errors identified from the transcript.', 'لم تُرصد أخطاء سريرية خطيرة في المحادثة.', lang));
    }
    const caseTitle = lang === 'AR' ? caseData.titleAr || caseData.titleEn : caseData.titleEn;
    const reportTitle = pickLang('OSCE Evaluation Report', 'تقرير تقييم OSCE', lang);
    const perfSummary = pickLang('Performance Summary', 'ملخص الأداء', lang);
    ...
    const overallLabel = pickLang('Overall Score', 'الدرجة الإجمالية', lang);
    const caseLabel = pickLang('Case', 'الحالة', lang);
    const reportFooter = pickLang(`Report generated from full session transcript (${messages.length} messages).`, `التقرير مُولَّد من محادثة الجلسة الكاملة (${messages.length} رسالة).`, lang);
    return {
        totalScore,
        communicationScore,
        historyTakingScore,
        ...
        recommendations: pickLang('Review systematic cardiac history (SOCRATES + red flags), rheumatic heart disease features, and structured OSCE closing with diagnosis and management plan.', 'راجع أخذ التاريخ القلبي المنهجي (SOCRATES + red flags)، وسمات مرض القلب الروماتيزمي، وختام OSCE منظم بالتشخيص وخطة الإدارة.', lang),
        idealApproach: pickLang(`Introduce yourself, confirm identity, explore ${caseData.chiefComplaint} using SOCRATES with exertional symptoms, ask about rheumatic fever and penicillin prophylaxis, complete examination with murmur description, then state ${caseData.finalDiagnosis} with echo referral and management plan.`, `قدّم نفسك، تأكد من الهوية، استكشف ${caseData.chiefComplaint} بـ SOCRATES مع أعراض المجهود، اسأل عن الحمى الروماتيزمية والبروفيلاكس بالبنسلين، أكمل الفحص مع وصف الـ murmur، ثم اذكر ${caseData.finalDiagnosis} مع إحالة echo وخطة إدارة.`, lang),
...
export async function getExaminerEvaluation(caseData, messages, lang = 'EN', context = {}, usageMeta) {
    ...
    const raw = await callOpenAISafe(chatMessages, settings.examinerModel, 0.3, 4096, () => JSON.stringify(mockResult), { feature: 'evaluation', userId: usageMeta?.userId, sessionId: usageMeta?.sessionId });
    try {
...
export function buildRealtimePatientInstructions(caseData, sessionLanguage) {
    const lang = sessionLanguage === 'EN' ? 'EN' : 'AR';
    const structuredContext = [
...
function patientActingAsDoctor(text) {
    return /ممكن تحكيلي|قولي إيه|قولّي إيه|قولي بالظبط|تحكيلي|هل عندك|عندك كحة|لما تتحرك|لما تتعب|tell me if|can you tell|what happens when|do you have|when you move/i.test(text);
}
function stripDoctorQuestionsFromPatient(text) {
...
export function sanitizeRealtimePatientTranscript(caseData, studentMessage, patientTranscript, sessionLanguage) {
    const lang = sessionLanguage === 'EN' ? 'EN' : 'AR';
...
export async function getPatientResponse(caseData, history, userMessage, language, options) {
    ...
    const studentTurn = history.filter((m) => m.role === 'STUDENT').length;
    const patientBehavior = options?.patientBehavior ??
    ...
    if (isGreetingOnly(normalizedMessage) &&
        ...
        const greeting = patientGreetingOnlyReply(caseData, resolvePatientLanguage(lang, normalizedMessage), normalizedMessage);
    ...
    */
    ...
    if (multiPart || !customPatientKnowledge || requiresGroundedDeterministicReply) {
        const deterministic = getDeterministicPatientResponse(caseData, normalizedMessage, lang, history, voiceTurn);
        if (deterministic !== null) {
            ...
            const hasCompleteDeterministicCoverage = !multiPart ||
    ...
    const provider = process.env.AI_PROVIDER || settings.provider;
    if (provider === 'mock' || provider === 'demo') {
        return finalizePatientReply(caseData, normalizedMessage, mockPatientResponse(caseData, normalizedMessage, lang, history), lang, history, voiceTurn);
    }
    const knowledgeContext = await getRoleKnowledgeContext({
        categoryId: caseData.categoryId,
        caseId: caseData.id,
    ...
    const multiPartRule = multiPart
        ? `\nMULTI-QUESTION MESSAGE: The doctor asked several questions in the same turn. Answer EVERY distinct question and clinical sub-question in one natural reply. This includes demographics AND clinical history such as onset, duration, triggers, relieving factors, associated symptoms, orthopnea/PND, edema, medications, past illnesses, hypertension, diabetes, allergies, family history, smoking and alcohol when asked. Do not choose only one "main" question and ignore the others. Answer only from the configured case background and approved patient knowledge; never invent missing medical facts. Stay fully in the session language and keep the reply natural rather than turning it into a checklist.${lang === 'AR'
            ...
            : ''}`
        : '';
    const systemPrompt = buildPatientSystemPrompt(caseData, lang, knowledgeContext, voiceTurn, studentTurn, patientBehavior) +
        multiPartRule +
    ...
    const voiceModel = process.env.OPENAI_VOICE_MODEL ||
    ...
    const maxTokens = voiceTurn
        ...
        : Math.min(Math.max(settings.maxTokens, multiPart ? CHAT_PATIENT_MULTI_MAX_TOKENS : CHAT_PATIENT_MAX_TOKENS), multiPart ? CHAT_PATIENT_MULTI_MAX_TOKENS : CHAT_PATIENT_MAX_TOKENS);
    ...
    const raw = await callOpenAISafe(messages, activeModel, temperature, maxTokens, () => voiceTurn
    ...
    const sanitized = sanitizePatientResponse(caseData, normalizedMessage, raw, lang, voiceTurn);
    const finalized = finalizePatientReply(caseData, normalizedMessage, sanitized, lang, history, voiceTurn);
...
function vivaQuestionKeywords(question) {
    return question
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, ' ')
        .split(/\s+/)
...
function stripModelAnswerPreamble(text) {
    return stripMarkdown(text)
        .replace(/^(?:the\s+)?(?:main\s+|key\s+|important\s+|expected\s+)?(?:alarm\s+)?(?:symptoms?|signs?|red\s*flags?|findings?|causes?|complications?|features?|points?|answers?)(?:\s+(?:of|for|in|to)\s+[^.:,;]{0,60})?\s*(?:include|are|comprise|consist of|:)\s*/i, '')
...
function isLabeledDefinitionPoint(point) {
...
function extractPointTerm(point) {
...
function shortFindingCue(value) {
    const cues = [
 ...
 */
function splitRegionLabeledFindings(text) {
    const cleaned = stripMarkdown(text).trim();
    if (!cleaned)
        return [];
    const re = /(?:^|[.!?]\s+|\n+)([A-Z][A-Za-z]*(?:\s+[A-Z][A-Za-z]*){0,3})\s*:\s+/g;
    const matches = [...cleaned.matchAll(re)].filter((m) => REGION_FINDING_LABELS.has(m[1].trim().toLowerCase()));
    if (matches.length < 2)
        return [];
    const points = [];
...
function atomizeClinicalFindings(value) {
    ...
    for (const sentence of source) {
        ...
        if ((/^(?:middle-aged|elderly|young|male|female|fully conscious|cooperative)\b/i.test(sentence) ||
            /(?:mildly\s+)?tachypneic|sitting upright|cooperative adolescent/i.test(sentence)) &&
            sentence.split(/\s+/).length <= 12 &&
            !/(?:edema|oedema|puffiness|swelling|scar|jaundice|icterus|distend|bulge|vein|lesion)/i.test(sentence)) {
            continue;
        }
        ...
        const scarSite = sentence.match(/^(.+?\bscar\b.+?)\s+in\s+(left\s+mid-?axillary\s+line)\b/i);
        if (scarSite) {
        ...
        if (/^(?:the\s+)?skin\b/i.test(sentence) ||
    ...
    if (unique.length === 0 && cleaned)
        return [cleaned];
    return unique.length >= 2 ? unique : cleaned ? [cleaned] : [];
}
function splitModelAnswerPoints(sampleAnswer) {
    ...
    const looksLikeMultiLabeled = structured.length >= 2 && structured.filter(isLabeledDefinitionPoint).length >= 2;
    ...
    if (structured.length >= 2) {
        ...
        const flattened = structured.flatMap((point) => {
            if (isLabeledDefinitionPoint(point))
    ...
    if (proseConcepts.length >= 2)
        return proseConcepts;
...
function splitProseCauseList(text) {
    ...
    const looksLikeProseSentence = /^(?:the\s+)?(?:patient|doctor|examiner|answer|aim|goal|purpose)\b/i.test(cleaned) ||
        /\bmust\b|\bshould\b|\bin order to\b|\bso that\b/i.test(cleaned);
    if (looksLikeProseSentence)
        return [];
    if (!/,|\band\b|\bor\b|\//i.test(cleaned))
        return [];
    const parts = cleaned
 ...
 */
function splitLongProseAnswer(text) {
...
function requiredLabelTerms(label) {
    ...
    if (lower.includes('left') && lower.includes('parasternal'))
        terms.push('left');
    if (terms.length === 0 && /\bapical\b/.test(lower))
        terms.push('apical');
    return [...new Set(terms)];
}
function requiredValueTerms(value) {
...
function normalizeVivaStudentText(text) {
    return text
        .normalize('NFKC')
        .replace(/[\u200B-\u200D\uFEFF\u00AD]/g, '')
        .replace(/[\u2018\u2019\u201A\u201B]/g, "'")
...
function colonStructuredPointIsCovered(studentLower, point, studentRawLower) {
    const cleaned = stripMarkdown(point);
    ...
    const labelMentioned = (!isRegionLabel &&
        labelWords.length > 0 &&
    ...
    if (!isRegionLabel && valueTerms.length > 0) {
        ...
        if (valueTerms.length <= 2 && valueHits.length >= 1)
            return true;
        if (valueHits.length >= 2 || valueHits.length / valueTerms.length >= 0.45)
            return true;
    }
...
function extractStudentDefinitionForLabel(studentLower, label) {
    const labelLower = label.toLowerCase().trim();
    if (labelLower.length < 3)
        return null;
    const escaped = labelLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const patterns = [
        new RegExp(`\\b${escaped}\\b\\s*(?:is|=|:|means|meaning|refers\\s+to|defined\\s+as|يعني|هي|هو)\\s+([^.;\\n]+)`, 'i'),
...
function definitionMatchesExpectedValue(studentDef, expectedValue, valueTerms) {
    ...
    for (const defLower of pools) {
        if (expectedPhrase.length >= 8 && defLower.includes(expectedPhrase))
            return true;
        if (valueTerms.length === 0) {
            ...
            if (hits.length >= 1 && hits.length / Math.max(expectedWords.length, 1) >= 0.4)
                return true;
            continue;
        }
...
function findWrongLabeledDefinitionFeedback(studentAnswer, sampleAnswer) {
    ...
    for (const point of points) {
        ...
        if (!studentDef)
            continue;
        const valueTerms = requiredValueTerms(value);
        if (definitionMatchesExpectedValue(studentDef, value, valueTerms))
            continue;
        const term = extractPointTerm(point);
        return (`Not quite — ${term} is not "${studentDef.trim()}". ` +
...
export function detectVivaStudentIntent(raw) {
    const text = normalizeVivaStudentText(raw);
    if (!text)
        return 'off_topic';
    if (studentGaveUpAnswer(text))
        return 'give_up';
    const lower = text.toLowerCase();
    // Avoid \\b with Arabic — it does not treat Arabic letters as word chars.
    if (/^(hint|help|coach|مساعدة|ساعدني|لمح|لمّح|تلميح|دربني|درّبني)(\s|$)/i.test(text) ||
        /\b(give\s+me\s+(a\s+)?hint|can\s+you\s+help|coach\s+me|teach\s+me|guide\s+me|what\s+should\s+i\s+(say|write|answer))\b/i.test(lower) ||
        /(عايز|عاوز)\s*(تلميح|مساعدة|hint|coach)/i.test(text)) {
        return 'hint';
    }
    if (/\b(repeat(\s+the)?\s+question|say\s+(it\s+)?again|ask\s+again)\b/i.test(lower) ||
        /^(repeat|again)(\s|$)/i.test(text) ||
        /^(أعد|كرر|قول\s*تاني|السؤال\s*تاني)/i.test(text) ||
        /(أعد|كرر).*(سؤال)/i.test(text)) {
        return 'repeat';
    }
    if (/\b(i\s+don'?t\s+understand|what\s+do\s+you\s+mean|clarify|rephrase|explain\s+the\s+question)\b/i.test(lower) ||
        /(مش\s*فاهم|مو\s*فاهم|وضح|اشرح\s*السؤال)/i.test(text)) {
        return 'clarify';
    }
    if ((/^(hi|hello|hey|thanks|thank\s+you|ok|okay)(\s|$)/i.test(text) ||
...
function feedbackLeaksMissingPoints(feedback, studentAnswer, sampleAnswer) {
    if (!feedback.trim() || !sampleAnswer.trim())
        return false;
    ...
    for (const point of missing) {
        ...
        if (combinedStudent.includes(term) || studentLower.includes(term))
            continue;
        // Distinctive multi-word clinical labels in feedback = leak.
        if (term.split(/\s+/).length >= 2 && fb.includes(term))
            return true;
...
function expandClinicalAliases(text) {
...
function tokensRoughlyMatch(studentToken, pointToken) {
    if (studentToken === pointToken)
        return true;
    ...
    let shared = 0;
    while (shared < shorter.length && shorter[shared] === longer[shared])
        shared += 1;
    if (shared >= 6) {
...
function studentNegatesTerm(studentLower, term) {
    const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return (new RegExp(`\\b(?:no|without|absent|denies?|negative\\s+for)\\s+(?:a\\s+|any\\s+|visible\\s+|obvious\\s+)?${escaped}\\b`, 'i').test(studentLower) ||
        new RegExp(`\\bthere(?:'s|\\s+is|\\s+are)\\s+no\\s+(?:a\\s+|any\\s+)?${escaped}\\b`, 'i').test(studentLower) ||
        new RegExp(`\\b${escaped}\\s+(?:is|are)\\s+(?:absent|not\\s+present|not\\s+seen|not\\s+visible)\\b`, 'i').test(studentLower));
}
/** True when the model point expects a POSITIVE finding (scar present), not "no X". */
function modelPointExpectsPositiveFinding(point) {
    const p = stripMarkdown(point).toLowerCase().trim();
    if (/^no\b/i.test(p))
        return false;
    if (/\bno\s+(?:precordial|dilated|chest\s+skin|pallor|localized|or)\b/i.test(p))
        return false;
    return true;
}
function examFindingAnchorCovered(studentLower, point) {
    ...
    for (const { pointHint, studentTest } of EXAM_FINDING_ANCHORS) {
        if (!pointHint.test(pointText))
            continue;
        if (!studentTest.test(studentLower))
            continue;
        if (/smooth|stretched|shiny/i.test(pointText)) {
            const hits = ['smooth', 'pale', 'stretched', 'shiny'].filter((w) => new RegExp(`\\b${w}\\b`, 'i').test(studentLower));
            if (hits.length < 2)
                continue;
        }
        // "there's no scar" must never credit a positive scar / chest-tube finding.
        if (/\bscar\b|chest\s+tube|thoracotomy|ندبة/i.test(pointText) &&
...
function pointIsCovered(studentLower, point, studentRawLower) {
    ...
    if (modelPointExpectsPositiveFinding(cleaned) &&
    ...
    if (examFindingAnchorCovered(studentLower, point))
        return true;
...
export function debugSplitVivaPoints(sampleAnswer) {
    return splitModelAnswerPoints(sampleAnswer);
}
export function debugScoreViva(studentAnswer, sampleAnswer) {
    return scoreAnswerAgainstModel(studentAnswer, sampleAnswer);
}
/** Test helper for strict physical-maneuver component grounding. */
export function debugScoreManeuver(studentAnswer, sampleAnswer) {
    return scoreManeuverAnswerAgainstExpected(studentAnswer, sampleAnswer);
}
export function debugEvaluateHistoryVivaLocal(studentAnswer, sampleAnswer, combinedStudentAnswer) {
    return evaluateHistoryVivaAnswerFromModel(studentAnswer, sampleAnswer, combinedStudentAnswer);
}
function scoreAnswerAgainstModel(studentAnswer, sampleAnswer) {
    const points = splitModelAnswerPoints(sampleAnswer);
    if (points.length === 0) {
    ...
    const missing = [];
    for (const point of points) {
        if (pointIsCovered(studentLower, point, studentRaw))
            matched.push(point);
        else
            missing.push(point);
    }
 ...
 */
function splitManeuverExpectedComponents(sampleAnswer) {
    ...
    for (const segment of segments) {
        ...
        if (/\bscar\b/i.test(segment)) {
            ...
            if (scarLocation)
                atoms.push(scarLocation);
            if (/chest\s+tube/i.test(segment))
                atoms.push('chest tube scar');
            else if (/thoracotomy/i.test(segment))
                atoms.push('thoracotomy scar');
        }
...
function maneuverPointKeywords(point) {
    return stripMarkdown(point)
        .toLowerCase()
        .replace(/[^a-z0-9\u0600-\u06ff\s]/g, ' ')
        .split(/\s+/)
        .filter((word) => (word.length >= 3 || word === 's1' || word === 's2') &&
        !VIVA_EVAL_STOP_WORDS.has(word) &&
        !['area', 'at', 'the', 'and', 'with', 'from', 'left', 'right'].includes(word));
}
function maneuverPointIsDemonstrated(studentAnswer, point) {
    const studentRaw = normalizeVivaStudentText(studentAnswer).toLowerCase();
    ...
    if (/\bscar\b/i.test(compactPoint)) {
        if (/thoracotomy|chest\s*tube|sternotomy/i.test(compactPoint) && !/thoracotomy|chest\s*tube|sternotomy/i.test(studentRaw))
            return false;
        if (/\bleft\b|\bright\b|lateral|anterior|posterior|chest/i.test(compactPoint) && !/\bleft\b|\bright\b|lateral|anterior|posterior|chest/i.test(studentRaw))
            return false;
        if (/scar\s+(?:present|seen|noted)/i.test(compactPoint))
            return /\bscar\b/i.test(student);
    }
    if (/radiat/i.test(compactPoint)) {
        return /radiat/i.test(student) && (!/axilla/i.test(compactPoint) || /axilla/i.test(student));
    }
    if (compactPoint.length >= 5 && student.includes(compactPoint))
        return true;
    if (examFindingAnchorCovered(student, point))
        return true;
    ...
    const hits = keywords.filter((word) => new RegExp(`\\b${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(student));
    return keywords.length === 1 ? hits.length === 1 : hits.length >= 2;
}
function scoreManeuverAnswerAgainstExpected(studentAnswer, sampleAnswer) {
...
export function getManeuverCompletionState(caseData, maneuverId, studentAnswers) {
    const physicalExam = parsePhysicalExamForm(caseData.physicalExam);
    const modelAnswer = physicalExam[maneuverId]?.trim() || '';
...
function shortPointLabel(point) {
    ...
    const clinical = words.filter((word) => word.length >= 5 &&
        !VIVA_EVAL_STOP_WORDS.has(word.toLowerCase()) &&
        !/^(include|symptoms?|alarm|findings?|features?|localized|diffuse|muscle|contraction|pain|release)$/i.test(word));
    if (clinical.length > 0)
        return clinical.slice(0, 3).join(' ').slice(0, 50);
    return cleaned.slice(0, 40).trim();
}
function pickVariedPhrase(seed, options) {
    let hash = 2166136261;
...
function praiseSinglePoint(label, remaining, seedExtra = '') {
...
function buildPartialCreditFeedback(matched, missing, options) {
    if (missing.length === 0) {
        ...
        if (labels.length >= 2) {
            const list = labels.length === 2
                ? `${labels[0]} and ${labels[1]}`
                : `${labels.slice(0, -1).join(', ')}, and ${labels[labels.length - 1]}`;
            return pickVariedPhrase(`done|${list}`, [
    ...
    const highlight = options?.newlyMatched && options.newlyMatched.length > 0 ? options.newlyMatched : matched;
    ...
    if (options?.noNewProgress) {
        return remaining === 1
            ? pickVariedPhrase(`noprog|1|${varietySeed}`, [
            ...
            : pickVariedPhrase(`noprog|n|${varietySeed}`, [
    ...
    if (options?.duplicateAttempt) {
        return remaining === 1
            ? pickVariedPhrase(`dup|1|${varietySeed}`, [
            ...
            : pickVariedPhrase(`dup|n|${varietySeed}`, [
 ...
 */
function safeProbeForMissingComponents(missing) {
    ...
    if (/precordial\s+bulge/.test(text))
        return 'Is there any precordial bulge?';
    if (/dilated\s+superficial\s+veins/.test(text))
        return 'Do you see any dilated superficial veins?';
    if (/chest\s+skin\s+lesions/.test(text))
        return 'Are there any chest skin lesions?';
    ...
    if (asksRadiation)
        return 'Where does the murmur radiate?';
    if (/\b(?:s1|s2)\b|heart\s+sound/.test(text))
        return 'How would you describe the heart sounds?';
    if (/intensity|loud|grade/.test(text))
        return 'How would you describe the intensity?';
    if (/timing|systolic|diastolic/.test(text))
        return 'What is the timing of the murmur?';
    if (/best\s+heard|auscultat|apex|area/.test(text))
        return 'Where is it best heard?';
    if (/lung|chest\s+exam/.test(text))
        return 'What would you expect on chest examination?';
    return 'What is one further relevant finding?';
}
function buildProgressiveManeuverFeedback(current, combined, sampleAnswer) {
    ...
    const previous = before ? scoreManeuverAnswerAgainstExpected(before, sampleAnswer) : { matched: [] };
    if (coverage.matched.length === 0 || coverage.missing.length === 0)
        return null;
    ...
    const mentionedUncreditedDomain = (/radiat/.test(missingText) && /radiat|carotid|axilla|towards|to\s+the/i.test(currentLower)) ||
...
function priorCombinedAnswer(combined, current) {
    const trimmedCombined = combined.trim();
    const trimmedCurrent = current.trim();
    if (!trimmedCurrent || trimmedCombined === trimmedCurrent)
        return '';
...
function evaluateHistoryVivaAnswerFromModel(studentAnswer, sampleAnswer, combinedStudentAnswer) {
    ...
    const duplicateAttempt = before.length > 0 &&
    ...
    if (words.length < 2 &&
        ...
        return {
            advance: false,
            feedback: 'That is quite brief. Give one clear clinical point, then we can build the rest of the answer.',
        };
    }
    ...
    if ((studentNegatesTerm(current.toLowerCase(), 'scar') ||
        studentNegatesTerm(current.toLowerCase(), 'ندبة')) &&
        splitModelAnswerPoints(sampleAnswer).some((p) => /\bscar\b|chest\s+tube|thoracotomy/i.test(p) && modelPointExpectsPositiveFinding(p))) {
        return {
            advance: false,
            feedback: 'Not quite — saying there is no scar is incorrect for this case. Look again at the clinical images and describe the positive findings you see.',
        };
    }
    return {
        advance: false,
        feedback: 'Not complete yet — try listing the main clinical points systematically, one by one. I will coach you as you go.',
    };
}
function mockEvaluateHistoryVivaAnswer(vivaQuestion, studentAnswer, sampleAnswer, combinedStudentAnswer) {
    ...
    if (words.length < 3) {
        return {
            advance: false,
            feedback: 'That is quite brief. Think about the key clinical point in the question and try again.',
        };
    }
...
function parseVivaAnswerEvaluation(raw, fallback) {
    try {
        ...
        if (typeof parsed.advance !== 'boolean')
            return fallback();
        const feedback = unwrapExaminerPlainText(typeof parsed.feedback === 'string' ? parsed.feedback.trim() : '');
        if (!feedback)
            return fallback();
        if (/question\s+\d+\s+of\s+\d+/i.test(feedback))
            return fallback();
...
async function evaluateOsceMeaningTurn(options) {
    const { caseData, question, studentAnswer, combinedAnswer, sampleAnswer, questionNumber, stationLabel, usageMeta, } = options;
    const settings = await getAISettings();
    const provider = process.env.AI_PROVIDER || settings.provider;
    ...
    if (intent === 'hint') {
        return {
            advance: false,
            feedback: 'Hint: focus on the underlying clinical concept — synonyms and plain English are fine. I will not mark this message as an answer attempt.',
        };
    }
    if (intent === 'repeat') {
    ...
    const modelAnswerBlock = sampleAnswer.trim()
        ...
        : '';
    const priorAttempts = combined && combined !== current
        ? combined
            .split('\n')
            .map((line) => line.trim())
            .filter((line) => line && line !== current)
        : [];
    const attemptBlock = priorAttempts.length > 0
    ...
    const qNumLine = typeof questionNumber === 'number' ? `\nVIVA QUESTION NUMBER: ${questionNumber}` : '';
    const messages = [
        {
            role: 'system',
...
${modelAnswerBlock}${coverageHint}${knowledgeContext}`,
    ...
    const raw = await callOpenAISafe(messages, settings.examinerModel, 0.2, 360, () => JSON.stringify(local), { feature: 'examiner_viva', userId: usageMeta?.userId, sessionId: usageMeta?.sessionId });
...
export async function evaluateHistoryVivaAnswer(caseData, vivaQuestion, questionNumber, studentAnswer, sampleAnswer = '', combinedStudentAnswer) {
    const combined = (combinedStudentAnswer ?? studentAnswer).trim();
...
export async function getExaminerVivaResponse(caseData, question, history, language = 'AR', usageMeta) {
    const lang = resolveExaminerLanguage(language, question);
    const settings = await getAISettings();
    const provider = process.env.AI_PROVIDER || settings.provider;
    const caseTitle = lang === 'AR' ? caseData.titleAr || caseData.titleEn : caseData.titleEn;
    if (provider === 'mock' || provider === 'demo') {
        return finalizeExaminerReply(lang === 'AR'
            ? `محاولة كويسة. في حالة ${caseTitle}، فكّر في التشخيصات التفريقية والتحاليل اللي بعد كده.`
            : `Good attempt. For this case (${caseTitle}), consider also discussing differential diagnoses and next investigation steps.`, lang);
    }
    const knowledgeContext = await getRoleKnowledgeContext({
        categoryId: caseData.categoryId,
        caseId: caseData.id,
...
5. ${examinerLangRule(lang)}${knowledgeContext}${adminSystemPromptSuffix(settings, lang, 'examiner')}`,
    ...
    const reply = await callOpenAISafe(messages, process.env.OPENAI_VOICE_MODEL || settings.examinerModel, Math.min(settings.temperature, 0.4), Math.min(settings.maxTokens, CHAT_EXAMINER_MAX_TOKENS), () => lang === 'AR'
        ? `محاولة كويسة. في حالة ${caseTitle}، فكّر في التشخيصات التفريقية والتحاليل اللي بعد كده.`
        : `Good attempt. For this case (${caseTitle}), consider also discussing differential diagnoses and next investigation steps.`, { feature: 'examiner_viva', userId: usageMeta?.userId, sessionId: usageMeta?.sessionId });
    return finalizeExaminerReply(unwrapExaminerPlainText(reply), lang);
}
function maneuverLabel(maneuverId, isArabic, stationConfigRaw) {
    return resolveManeuverLabel(maneuverId, parseStationConfig(stationConfigRaw), isArabic ? 'ar' : 'en');
}
export function getManeuverOpeningMessage(_caseData, maneuverId, _language, stationConfig) {
    if (stationConfig) {
...
export async function getManeuverExaminerResponse(caseData, maneuverId, question, history, _language, usageMeta, writtenProgressiveProbing = false) {
    const name = maneuverLabel(maneuverId, false, caseData.stationConfig);
    const physicalExam = parsePhysicalExamForm(caseData.physicalExam);
    const maneuverFindings = physicalExam[maneuverId]?.trim() || '';
    ...
    if (studentGaveUpAnswer(current)) {
        if (maneuverFindings) {
            return unwrapExaminerPlainText(`No problem — here are the expected ${name} findings:\n${maneuverFindings}`);
        }
        return unwrapExaminerPlainText(`That's fine — it's good to say when you're unsure. For ${name}, review the key clinical signs linked to this case and we can continue.`);
    }
    ...
    if (evaluation.advance) {
        return unwrapExaminerPlainText(`${evaluation.feedback} Let's move on when you're ready — you can refine technique or continue to the next examination step.`);
    }
...
function studentGaveUpAnswer(answer) {
