...
import { buildOtpEmailHtml, buildOtpEmailText, buildPasswordResetEmailHtml, buildPasswordResetEmailText, getOtpEmailSubject, getPasswordResetEmailSubject, getEmailSiteUrl, normalizeEmailLang, } from './emailTemplates.js';
...
export async function sendOtpEmail(to, code, firstName, lang = 'en') {
...
export async function sendPasswordResetEmail(to, firstName, resetToken, lang = 'en') {
    const fromEmail = process.env.SMTP_FROM || process.env.SMTP_USER;
    const fromName = process.env.SMTP_FROM_NAME || 'Synoza';
    const emailLang = normalizeEmailLang(lang);
