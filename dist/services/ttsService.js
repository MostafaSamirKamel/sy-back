...
function looksAlreadyColloquial(text) {
...
export async function synthesizeSpeech(text, lang) {
    ...
    const trimmed = (isArabic && !looksAlreadyColloquial(raw) ? toEgyptianColloquial(raw) : raw).trim();
    if (!trimmed) {
    ...
    const response = await client.audio.speech.create({
        ...
        ...(isArabic
            ? {
                instructions: process.env.OPENAI_TTS_INSTRUCTIONS_AR ||
            ...
            : {
                instructions: 'Speak the provided text accurately and clearly without omitting or inventing content.',
            }),
    });
