...
export function calculateUniversityRank(currentUserId, cohort) {
    const current = cohort.find((candidate) => candidate.id === currentUserId);
    if (!current || cohort.length === 0)
        return null;
...
export function selectUniversityCohort(current, candidates) {
...
export async function getUniversityRank(userId) {
