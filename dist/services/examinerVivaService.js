import { detectVivaStudentIntent, evaluateHistoryVivaAnswer, unwrapExaminerPlainText, } from "./aiService.js";
export const HISTORY_EXAMINER_STAGE = "history:examiner";
export const VIVA_QUESTIONS_PER_SESSION = 5;
const VIVA_CLOSING_EN = "Thank you. That completes the examiner viva for this station. You may continue with the rest of the OSCE. Good luck!";
const VIVA_CLOSING_AR = "شكراً. كده خلصنا أسئلة الـ examiner في المحطة دي. تقدر تكمل باقي الـ OSCE. بالتوفيق!";
/** @deprecated Prefer getVivaClosing(lang) */
export const VIVA_CLOSING = VIVA_CLOSING_EN;
export function getVivaClosing(language) {
    const code = String(language || 'EN').toUpperCase();
    if (code === 'EN')
        return VIVA_CLOSING_EN;
    // AR and AUTO → Arabic closing with بالتوفيق
    return VIVA_CLOSING_AR;
}
export function isVivaClosingText(content) {
    const text = content || '';
    return (text.includes('completes the examiner viva') ||
...
function parseCaseExaminerQuestions(caseData) {
    try {
        const parsed = JSON.parse(caseData.examinerQuestions || "[]");
...
export function pickVivaQuestionsForSession(sessionId, caseData) {
...
export function isHistoryExaminerVivaStage(stage, maneuverId) {
...
export function parseVivaQuestionNumber(content) {
...
function messageContainsAnyQuestion(content, questions) {
 ...
 */
export function getCurrentVivaQuestionNumber(messages, stage, questions = []) {
    let maxLegacy = 0;
    for (const message of messages) {
        if (message.stage !== stage || message.role !== "EXAMINER")
            continue;
        const legacy = parseVivaQuestionNumber(message.content);
        if (legacy && legacy > maxLegacy)
            maxLegacy = legacy;
    }
    if (maxLegacy > 0)
        return maxLegacy;
...
export function studentGaveUp(answer) {
    const normalized = answer.trim();
    if (!normalized)
        return false;
    return GAVE_UP_PATTERNS.some((pattern) => pattern.test(normalized));
}
export function isExaminerVivaComplete(messages, stage) {
    return messages.some((m) => m.stage === stage &&
        m.role === "EXAMINER" &&
        isVivaClosingText(m.content));
}
export function buildExaminerVivaOpening(sessionId, caseData) {
...
export function getCumulativeStudentAnswerForCurrentQuestion(messages, stage, latestAnswer = "", currentQuestionText = "") {
    let questionStartIndex = -1;
    for (let i = messages.length - 1; i >= 0; i -= 1) {
        const message = messages[i];
        if (message.stage !== stage || message.role !== "EXAMINER")
            continue;
        const isCurrentQuestion = (currentQuestionText &&
...
function buildGaveUpFeedback(sampleAnswer, language) {
...
function buildHintFeedback(question, sampleAnswer) {
    const model = sampleAnswer.trim();
    if (!model) {
    ...
    const topic = label && label.length <= 40 && label.split(/\s+/).length <= 5
        ? label
...
export async function respondToHistoryVivaAnswer(sessionId, caseData, messages, stage, studentAnswer, language) {
    ...
    const { combined: combinedStudentAnswer } = getCumulativeStudentAnswerForCurrentQuestion(messages, stage, studentAnswer, currentQuestion.question);
    ...
    const evaluation = intent === 'give_up' || studentGaveUp(studentAnswer)
        ...
        : await evaluateHistoryVivaAnswer(caseData, currentQuestion.question, questionNumber, studentAnswer, currentQuestion.sampleAnswer, combinedStudentAnswer);
